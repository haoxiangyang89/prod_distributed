def sp_global(type):
    ...
    if type == 'item':
        ...
        if gbv.ite > 0:
            ...
            gbv.Ksi += 1.6 * gbv.rho * (gbv.R - gbv.RC1)
            gbv.Eta += 1.6 * gbv.rho * (gbv.R - gbv.RC2)
    ...
    if type == 'plant':
        ...
        prob.setObjective(gp.quicksum(gbv.Lam[i][j][t] * z[i][j][t] for i in range(gbv.K) for j in range(gbv.M)
                                      for t in range(gbv.N)) + gbv.rho / 2 *
                          gp.quicksum((z[i][j][t] - gbv.ZC[i][j][t]) ** 2 for i in range(gbv.K)
        ...
        gbv.S = np.maximum(0, gbv.SC1 + gbv.SC2 - (gbv.Eta + gbv.Xi) / gbv.rho) / 2

        if gbv.ite > 0:
            gbv.Lam += 1.6 * gbv.rho * (gbv.Z - gbv.ZC)
            gbv.Eta += 1.6 * gbv.rho * (gbv.S - gbv.SC1)
            gbv.Xi += 1.6 * gbv.rho * (gbv.S - gbv.SC2)
