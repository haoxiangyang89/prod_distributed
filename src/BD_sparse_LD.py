...
def pi_iter(k, data, x_vals, theta_vals):
    ...
    if ((sub_obj - theta_vals[k]) > 1e-4) and (sub_obj > 1.00001 * theta_vals[k]):
        ...
        slp = gp.Model()
        slp.setParam("OutputFlag",0)
        thetav_level = slp.addVar(name="thetav")
        pi_level = slp.addVars(prod_key, period_df.I, lb=-float('inf'), name="beta")
        pi_level_abs = slp.addVars(prod_key, period_df.I, lb=0.0, name="beta_abs")
        level_con = slp.addConstr(rho * thetav_level - gp.quicksum(pi_level_abs[i,j,t] for i,j in prod_key for t in period_df.I) >= 0)
        slp.setObjective(gp.quicksum(pi_level_abs[i, j, t] for i,j in prod_key for t in period_df.I), GRB.MINIMIZE)
        slp.addConstrs(pi_level_abs[i,j,t] >= pi_level[i,j,t] for i,j in prod_key for t in period_df.I)
        slp.addConstrs(pi_level_abs[i,j,t] >= -pi_level[i,j,t] for i,j in prod_key for t in period_df.I)
        slp.update()
        ...
        while (ub_smcp > 1.01 * current_lb) and (iter_num <= 500):
            ...
            if (ub_smcp > 1.01 * current_lb):
                ...
                slp.addConstr(thetav_level <= pi_obj + gp.quicksum(x_grad_vals[i,j,t] * (pi_level[i,j,t] - pi_hat[i,j,t]) for i,j in prod_key for t in period_df.I))
                slp.setObjective(gp.quicksum(
                        (pi_level[i, j, t] - best_dual[i, j, t]) ** 2 for i, j in prod_key for t in
                        period_df.I), GRB.MINIMIZE)
                level = alpha * ub_smcp + (1-alpha) * current_lb
                slp.remove(level_con)
                level_con = slp.addConstr(rho * thetav_level - gp.quicksum(pi_level_abs[i,j,t] for i,j in prod_key for t in period_df.I) >= level)
                slp.update()
                slp.optimize()
                if slp.Status == 12:
                    slp.setParam("NumericFocus",3)
                    slp.update()
                    slp.optimize()
                try:
                    for i,j in prod_key:
                        for t in period_df.I:
                            if abs(pi_level[i,j,t].X) > 1e-4:
...
def master_sub_loop(gbv):
    ...
    while True:
        ...
        # optimality cut
        with get_context("spawn").Pool(processes=6) as pool:   # 30
        #with Pool(processes=6) as pool:
            sub_opt_results = pool.map(partial(pi_iter, data=gbv, x_vals=x_vals, theta_vals=theta_vals), range(len(decomp_units)))
