...
def pi_iter(k, data, x_vals, theta_vals, lag_cuts=True, feas_cuts=True):
        ...
        if lag_cuts:
            ...
            while (ub_smcp > 1.01 * current_lb) and (iter_num <= 500):
                ...
                if lb_cand > current_lb:
                    current_lb = lb_cand
                    best_dual = {(i,j,t):(pi_hat[i,j,t] if abs(pi_hat[i,j,t]) > 1e-4 else 0) for i,j in prod_key for t in period_df.I}
    ...
    if sol_method == "master_sub_loop":
        ...
        while True:
            ...
            #with Pool(processes=6) as pool:
                sub_opt_results = pool.map(partial(pi_iter, data=gbv, x_vals=x_vals, theta_vals=theta_vals, lag_cuts=True, feas_cuts=False), range(len(decomp_units)))
            ub_cand = 0
