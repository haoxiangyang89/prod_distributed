...
def pi_iter(k, x_vals, theta_vals, lag_cuts=True, feas_cuts=True, level_lambda=0.5, level_mu=0.8):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        sub_opt_results = pool.imap(partial(pi_iter, x_vals=x_vals, theta_vals=theta_vals, lag_cuts=False, feas_cuts=True), range(len(decomp_units)))
        ub_cand = 0
        for item in sub_opt_results:
            ...
            if feas_gen:
                if np.sum(np.abs(np.floor(feas_dual))) > 0:
                    mp.addConstr(gp.quicksum(np.floor(feas_dual[i, j, t]) * mp.getVarByName(f"x[{i},{j},{t}]") for i, j in prod_key for t in period_df.I) <=
                        np.floor(sum(feas_dual[i, j, t] *  x_vals[i, j, t] for i, j in prod_key for t in period_df.I) - feas_value))
                else:
