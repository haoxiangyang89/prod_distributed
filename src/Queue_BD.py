# Queue implementation, assign subproblems to workers, each worker build models of the assigned subproblems in the beginning and store in cache
...
def master_prob(relax_option=False):
    ...
    mp._x, mp._theta = x, theta
    if level_obj_norm == 1:
        mp._xd = x_dev

    return mp

def build_sub_model(k, sparse_cuts=False):
    ...
    #sp.Params.Crossover = -1  # faster for LPs
    ...
    # fix the local variables to their global counterparts' values, rhs initialzied to 0
    sub_fix_x = sp.addConstrs(((xCi[i, j, t] + x_im_p[i,j,t] - x_im_m[i,j,t]) == 0 for i, j in prod_key_used for t in period_df.I
        ), name="sub_fix_x")
    
    # Store references for fast access
    #sp._x, sp._d1, sp._d2, sp._constrs = x, d1, d2, sub_fix_x
    #sp._constrs, sp._xp, sp._xm, sp._u, sp._v, sp._s, sp._UnitIndex, sp._TransitList = sub_fix_x, x_im_p, x_im_m, ui, vi, si, unit_ind_list, transit_list_i
    #return sp
...
def partition_indices_round_robin(n_subs, n_parts):  # assign LPs to workers
    parts = [[] for _ in range(n_parts)]
    for idx in range(n_subs):
        parts[idx % n_parts].append(idx)
    return parts

def worker_cache(worker_id, assigned_indices, in_queue, out_queue, cache_size):
    models = {}  # cache for models
    for lp_idx in assigned_indices[:min(cache_size, len(assigned_indices))]:
        models[lp_idx] = build_sub_model(lp_idx, sparse_cuts=sparse_cut_flag)
    ...
    while True:
        ...
        for lp_idx in assigned_indices:
            # Lazy build, maintain cache size
            if lp_idx in models:
                model_data = models[lp_idx]
            else:
                model_data = build_sub_model(lp_idx, sparse_cuts=sparse_cut_flag)

            model = model_data["sp"]
            sub_fix_x = model_data["sub_fix_x"]
            xp = model_data["x_im_p"]
            xm = model_data["x_im_m"]
            u = model_data["ui"]
            v = model_data["vi"]
            s = model_data["si"]
            unit_ind_list = model_data["unit_ind_list"]
            transit_list_i = model_data["transit_list_i"]

            keys = [(i, j, t) for i, j, t in sub_fix_x]
            # updata RHS of sub_fix_x
            for i, j, t in keys:
                sub_fix_x[i, j, t].RHS = rhs[i, j, t] * lot_size[i, j]  # update each constraint
            
            # feasibility subproblem
            obj = gp.quicksum(xp[i, j, t] + xm[i, j, t] for i, j, t in keys)
            model.setObjective(obj, GRB.MINIMIZE)
            model.optimize()
            if model.ObjVal > 1e-2:
                feas_gen = True
                feas_value = model.ObjVal
                feas_cut_coeff = {}
                for i, j, t in keys:                
            ...
            obj = gp.quicksum(
            holding_cost[i] * v[i, j, t] for i in unit_ind_list for j in plant_df.I for t in period_df.I) + \
            gp.quicksum(penalty_cost[i] * u[i, t] for i in unit_ind_list for t in period_df.I) + \
            gp.quicksum(transit_cost[transit_list_i.Tr[l]] * s[transit_list_i.Tr[l], t] for l in transit_list_i.index for t in period_df.I) + \
            gp.quicksum(penalty_mag * (xp[i, j, t] + xm[i, j, t]) for i,j,t in keys)
            model.setObjective(obj, GRB.MINIMIZE)
            model.optimize()
            sub_obj = model.ObjVal
            if ((sub_obj - theta[lp_idx]) > 1e-4) and (sub_obj > 1.00001 * theta[lp_idx]):
                cut_gen = True
                dual_coeff = {}
                for i, j, t in keys:
                    if abs(sub_fix_x[i, j, t].Pi * lot_size[i,j]) > 1e-4:
            ...
            results_for_iter[lp_idx] = [cut_gen, sub_obj, dual_coeff, feas_gen, feas_cut_coeff, feas_value]

            if not lp_idx in models:
                try:
                    model.dispose()
                except:
                    pass
                ...
                del model_data
                del model
                del sub_fix_x, xp, xm, u, v, s
                del unit_ind_list, transit_list_i
                del keys
                gc.collect()
        out_queue.put(results_for_iter)

if __name__ == "__main__":
    # read in data
    data_folder = "../test_dataset/small"   #small/medium_D0_0418/medium_D1_0222/medium_D4_0801
    ...
    penalty_mag = len(period_df.I) * np.max(list(penalty_cost.values()))  # penalty in adjusted sub-opt problem 
    ...
    # Accelerate technique 5: Generate sparse cut
    ...
    # print out result
    result_out_path = "Queue_" + data_folder.split("/")[-1] + "_full.txt"
    ...
    if lp_warm_flag:
        mp = master_prob(relax_option=True)
    else:
        mp = master_prob(relax_option=False)
    ...
    #    if cut_opt == "multi_cut":
        level_constr = mp.addConstr(
            gp.quicksum(mp._theta[i] for i in range(len(decomp_units))) <= level)
    #     elif cut_opt == "single_cut":
    #         level_constr = mp.addConstr(mp.getVarByName(f"theta") <= level)
        if level_obj_norm == 1:
            x_dev_constr1 = mp.addConstrs(
                            mp._xd[i,j,t] + mp._x[i,j,t] >= best_incum[i, j, t] for i, j in prod_key for t
                            in period_df.I)
    ...
    num_workers = min(30, len(decomp_units))

    # partition LP indices
    assignments = partition_indices_round_robin(len(decomp_units), num_workers)

    # cache size
    cache_size = math.ceil(len(decomp_units) / num_workers)

    # set up multiprocessing
    in_queues = [Queue() for _ in range(num_workers)]
    out_queue = Queue(maxsize=num_workers)
    workers = []
    for w in range(num_workers):
        assigned = assignments[w]
        p = Process(target=worker_cache, args=(w, assigned, in_queues[w], out_queue, cache_size))
        p.start()
        workers.append(p)
    ...
    if lp_warm_flag:
        ...
        while True:
            ...
            if not reg_BD_flag:
                ...
                mp.setObjective(gp.quicksum(mp._theta[i] for i in range(len(decomp_units))))
                #mp.update()
                mp.optimize()
                mp_time = time.time() - mp_start_time

                x_vals = {xKey: mp._x[xKey].X for xKey in mp._x}
                theta_vals = {thetaKey: mp._theta[thetaKey].X for thetaKey in mp._theta}
            ...
            # Regularized Benders
                ...
                if level_obj_norm == 2:
                    obj = gp.quicksum(
                        (mp._x[i,j,t] - best_incum[i, j, t]) ** 2 for i, j in prod_key for t in
                        period_df.I)
                    ...
                    obj = gp.quicksum(
                        mp._xd[i,j,t] for i, j in prod_key for t in period_df.I)
            ...
            # simultaneous generation of cuts
            if feas_opt_cut_flag:
                sub_start_time = time.time()
                # send tasks
                for w in range(num_workers):
                    in_queues[w].put((x_vals, theta_vals))
                # collect results
                sub_results = {}
                for w in range(num_workers):
                    w_res = out_queue.get()
                    sub_results.update(w_res)
                sub_time = time.time() - sub_start_time

                ub_cand = 0
                feas_sol_obtained = True
                for k in range(len(decomp_units)):
                    cut_gen, sub_obj, sub_dual, feas_gen, feas_dual, feas_value = sub_results[k]
                    ...
                    if feas_gen:
                        mp.addConstr(0 >= feas_value + gp.quicksum(feas_dual[i, j, t] * (mp._x[i,j,t] - x_vals[i, j, t])
                                for i, j, t in feas_dual.keys()))

                    if cut_gen:
                        mp.addConstr(mp._theta[k] >= sub_obj + gp.quicksum(
                            sub_dual[i, j, t] * (mp._x[i,j,t] - x_vals[i, j, t])
            ...
            # Conventional Benders: not generate optimality cut until a feasible solution is obtained
                ...
                if feas_sol_obtained:
                    ...
                    for item in sub_opt_results:
                        ...
                        if cut_gen:
                            mp.addConstr(mp._theta[k] >= pi_obj + gp.quicksum(
                                sub_dual[i, j, t] * (mp._x[i,j,t] - x_vals[i, j, t])
                                for i, j, t in sub_dual.keys()))
                else:
                    ub_cand = math.inf
        ...
        wr = "\nLP UB list: " + str(ubList) + "\nLP LB list: " + str(lbList) + "\nTotal LP Master/Sub prob: " + str((round(total_mp_time, 2), round(total_subp_time, 2))) + "\n"
        wr_s = open(result_out_path, "a")
        wr_s.write(wr)
        wr_s.close()
    ...
    # Clean up
    for w in range(num_workers):
        in_queues[w].put(None)
    for p in workers:
        p.join()
    end = time.time()
    
    wr = f"\nBest UB found in iteration {best_ub_iter}."
    wr += "\nUB list: " + str(ubList) + "\nLB list: " + str(lbList) + "\nTime evolution: " + str(time_list) + "\nTotal MIP Master/Sub prob: " + str((round(total_mp_time, 2), round(total_subp_time, 2)))
    if lp_warm_flag:
