...
def x_solve(dummy_item_index, rho_sep, global_vars, quad_penalty=True, relax_option=True, penalty_mag=1e5, fixed_param=True,
            fixed_lag=6):
    '''
    ...
    prob.addConstrs((gp.quicksum(
        ...
        if (j_iter == j) and (ct == ct_iter)) <= gbv.max_cap[ct, j][t]
                     for ct, j in gbv.max_cap.keys() for t in gbv.max_cap[ct, j].keys() if (ct, j) in unit_cap_ct_j),
                    name='capacity')
    '''
    prob.addConstrs((gp.quicksum(
        gbv.unit_cap[i_iter, j_iter, ct_iter] * xCi[i_iter, j_iter, t] for i_iter, j_iter, ct_iter in gbv.unit_cap.keys()
        if (j_iter == j) and (ct == ct_iter)) <= gbv.max_cap[ct, j][t]
    ...
    #    name='r_ub_rev_ini')
    prob.addConstrs(
        (rCi[jta] <= gbv.init_inv.get((jta[2][0], jta[0]), 0) for jta in gbv.alt_list if jta[1] == 1),
        name='r_ub_rev_ini')
    ...
    if quad_penalty:
        ...
        gvar_ind = 0
        obj_local_part += rho_sep[dummy_item_index] / 2 * gp.quicksum(
            (global_vars["value"][gvar_ind][xi_ind] - xCi[
                global_var_const["indices"][gvar_ind][xi_ind]]) ** 2 \
            for xi_ind in global_ind_i[gvar_ind])

        gvar_ind = 1
        obj_local_part += rho_sep[dummy_item_index] / 2 * gp.quicksum(
            (global_vars["value"][gvar_ind][ri_ind] - rCi[
                global_var_const["indices"][gvar_ind][ri_ind]]) ** 2 \
    ...
    '''
    if (2690, 53) in prod_plant_i:
        wr = "\nSub Item " + str(item_ind) + " solution of x[2690,53,3] + x[289,53,3] + x[2689,53,3] : " + str(xCi[2690, 53, 3].X) + ", "
        try:
            wr += str(xCi[289, 53, 3].X) + ", " + str(xCi[2689, 53, 3].X) + "\n"
        except:
            if (289, 53) not in prod_plant_i:
                wr += "key (289, 53) not exists, "
            elif (289, 53) in prod_plant_i:
                wr += "(289, 53) " + str(xCi[289, 53, 3].X) + ", "

            if (2689, 53) not in prod_plant_i:
                wr += "key (2689, 53) not exists, "
            elif (2689, 53) in prod_plant_i:
                wr += "(2689, 53) " + str(xCi[2689, 53, 3].X) + ", "
...
def x_solve_ub(dummy_item_index, global_vars, relax_option=True, penalty_mag=1e5):
    ...
    for l in transit_list_i:
        for t in range(min(gbv.period_list) - gbv.transit_time[(item_ind,) + l], min(gbv.period_list)):
            si[l + (t,)] = 0.0  # initial transportation set to 0
    for j in gbv.plant_list:
        vi[j, 0] = gbv.init_inv.get((item_ind, j), 0.0)  # initial inventory set to given values
    for i, j in prod_plant_i:
        for t in range(min(gbv.period_list) - int(gbv.lead_time[i, j]), min(gbv.period_list)):
            xCi[i, j, t] = 0.0  # initial production set to 0
...
def z_solve_lp(local_results, rho_sep, global_vars, var_threshold=1e-5):
    '''
    ...
    for gvar_ind in range(len(global_var_const["name"])):
        global_array = np.zeros(len(local_dict["value"][gvar_ind]))
        # global_counter = np.zeros(len(local_dict["value"][gvar_ind]))
        rho_total = np.zeros(len(local_dict["value"][gvar_ind]))
        for i in range(I):
            if len(global_ind[i][gvar_ind]) > 0:
                # calculate the local information to update the global variables
                local_array = np.zeros(len(local_dict["value"][gvar_ind]))
                ...
                # the dual value for the non-participating global variables is zero
                #global_array += (rho * local_array - global_vars["dual"][gvar_ind][i])
                global_array += rho_sep[i] * local_array
                rho_total[global_ind[i][gvar_ind]] += rho_sep[i]
        ...
        #global_var_pre_proc = np.maximum(global_array / (rho * global_counter[gvar_ind]), 0.0)
        global_var_pre_proc = np.maximum(global_array / rho_total, 0.0)
        # eliminate values that are very close to zero
        global_var_post_proc = np.where(global_var_pre_proc < var_threshold, 0.0, global_var_pre_proc)
        # calculate the dual residual
        for i in range(I):
            residual_list = rho_sep[i] * (global_var_post_proc[global_ind[i][gvar_ind]] - global_vars["value"][gvar_ind][
                global_ind[i][gvar_ind]])
            dual_residual_sqr += sum(residual_list ** 2)
            #global_combined.extend(global_var_post_proc[global_ind[i][gvar_ind]])
    ...
    return math.sqrt(dual_residual_sqr), local_dict, 0


def pi_solve(local_results, rho_sep, global_vars, var_threshold=1e-5, adj_coeff=1.0):  #adj_coeff=1.6
    '''
    local_results: the output from the x problem
    global_ind: a list with each item corresponding to the indices of the global variables selected for the sub
    global_var: a list global variable handles
    ...
    for gvar_ind in range(len(global_var_const["name"])):
        for i in range(I):
            if len(global_ind[i][gvar_ind]) > 0:
                ...
                local_dict["dual"][gvar_ind][i] += adj_coeff * rho_sep[i] * residual_list
    ...
    return math.sqrt(primal_residual_sqr), local_dict


def extensive_prob(solve_option=True, relax_option=True, penalty_mag=1e5):
    ...
    z_m = ext_prob.addVars(gbv.item_list, gbv.plant_list, gbv.period_list, lb=0.0, name="z_m")
    ...
    ext_prob.setObjective(
        gp.quicksum(
            gbv.holding_cost[i] * v[i, j, t] + penalty_mag * (z_m[i, j, t] + yUO_m[i, j, t]) for i in gbv.item_list for j in gbv.plant_list for t in gbv.period_list) +
...
if __name__ == '__main__':
    ...
    iter_no = 0
    primal_residual_sqr = hparams.init_pr
    dual_residual_sqr = hparams.init_dr

    # ACADMM parameter
    iter_no_0 = 0
    stop_thres = 1e-3
    rho_up_freq = 2
    safety_guard_thres = 0.2
    conv_const = 1e10
    ...
    UB = math.inf
    LB = -math.inf
    rho_sep = [10.0] * len(gbv.item_list)  # rho = 10.0
    ...
    wr = "\nExtensive problem objective : " + str(ext_prob.ObjVal)
    wr_s = open("results.txt", "a")
    wr_s.write(wr)
    wr_s.close()
    ...
    wr = "\nSum of dual: "
    for gvar_ind in range(len(global_var_const["name"])):
        for var_ind in range(len(global_vars["value"][gvar_ind])):
            dual_sum = 0
            for i in range(len(gbv.item_list)):
                dual_sum += global_vars["dual"][gvar_ind][i][var_ind]
            if abs(dual_sum) > 1e-5:
                wr += "\n" + str(dual_sum)
    wr_s = open("results.txt", "a")
    wr_s.write(wr)
    wr_s.close()
    ...
    #    partial(x_solve, rho_sep=rho_sep, global_vars=global_vars, quad_penalty=True, fixed_param=False),
    #    range(len(gbv.item_list)))
    #dual_residual_sqr, global_vars, z_time = z_solve(local_results, rho_sep, global_vars)
    #primal_residual_sqr, global_vars = pi_solve(local_results, rho_sep, global_vars)
    
    '''
    x_ind = [(x_prod[0], x_prod[1], t) for x_prod in gbv.prod_key for t in gbv.period_list]
    ind_1 = x_ind.index((2690, 53, 3))
    ind_2 = x_ind.index((289, 53, 3))
    ind_3 = x_ind.index((2689, 53, 3))
    wr = "\nAfter z problem x[2690,53,3] + x[289,53,3] + x[2689,53,3] : " + str(
        global_vars["value"][0][ind_1]) + ", " + str(global_vars["value"][0][ind_2]) + ", " + str(
        global_vars["value"][0][ind_3]) + "\n"
    wr_s = open("results.txt", "a")
    wr_s.write(wr)
    wr_s.close()
    '''
    '''
    # upper bound and lower bound calculation of initialization
    ...
    global_vars_before = copy.deepcopy(global_vars)

    ini_start = time.time()
    local_results = pool.map(partial(x_solve, rho_sep=rho_sep, global_vars=global_vars, quad_penalty=True, fixed_param=False),
                             range(len(gbv.item_list)))
    dual_residual, global_vars, z_time = z_solve(local_results, rho_sep, global_vars)
    primal_residual, global_vars = pi_solve(local_results, rho_sep, global_vars)
    ini_end = time.time()

    # hat variables initialization
    local_vars_0 = copy.deepcopy(local_results)
    global_var_0 = copy.deepcopy(global_vars["value"])
    dual_0 = copy.deepcopy(global_vars["dual"])
    hat_dual = copy.deepcopy(global_vars_before["dual"])
    for gvar_ind in range(len(global_var_const["name"])):
        for i in range(len(gbv.item_list)):
            # locally update hat_dual
            local_array = np.array(local_results[i][gvar_ind])
            local_array = np.where(local_array < var_threshold, 0.0, local_array)

            hat_dual[gvar_ind][i][global_ind[i][gvar_ind]] += adj_coeff * rho_sep[i] * (
                    global_vars_before["value"][gvar_ind][global_ind[i][gvar_ind]] - local_array)
    hat_dual_0 = copy.deepcopy(hat_dual)
    ...
    for gvar_ind in range(len(global_var_const["name"])):
        for i in range(len(local_results)):
            p_tol_1 += sum([u ** 2 for u in local_results[i][gvar_ind]])
            p_tol_2 += sum([v ** 2 for v in global_vars["value"][gvar_ind][global_ind[i][gvar_ind]]])

            d_tol += sum([lamb ** 2 for lamb in global_vars["dual"][gvar_ind][i][global_ind[i][gvar_ind]]])
    primal_residual_tol = stop_thres * max(math.sqrt(p_tol_1), math.sqrt(p_tol_2))
    dual_residual_tol = stop_thres * math.sqrt(d_tol)
    ...
    while primal_residual > primal_residual_tol or dual_residual > dual_residual_tol:
        ...
        local_results = pool.map(partial(x_solve, rho_sep=rho_sep, global_vars=global_vars, fixed_param=False), range(len(gbv.item_list)))
        ...
        print("------- Iteration {}, Primal Residual = {}, Dual Residual = {}, Rho min/max = {}, iter_time = {}".format(iter_no,
                                                                                                                np.round(
                                                                                                                    math.sqrt(primal_residual_sqr),
                                                                                                                    2),
                                                                                                                np.round(
                                                                                                                    math.sqrt(dual_residual_sqr),
                                                                                                                    2),
                                                                                                                (min(rho_sep), max(rho_sep)),
                                                                                                                np.round(
                                                                                                                    iter_elapsed_time,
                                                                                                                    2)))

        # update the rho penalty term
        if iter_no % rho_up_freq == 1:
            hat_dual = copy.deepcopy(global_vars_before["dual"])
            for gvar_ind in range(len(global_var_const["name"])):
                for i in range(len(gbv.item_list)):
                    # locally update hat_dual
            ...
            wr = "\nrho update parameters: " + str(iter_no)
            for i in range(len(gbv.item_list)):
                upper_SD_alpha = 0
                lower_SD_alpha = 0  #
                lower_MG_alpha = 0
                ...
                lower_MG_beta = 0
                for gvar_ind in range(len(global_var_const["name"])):
                    upper_SD_alpha += sum((hat_dual[gvar_ind][i] - hat_dual_0[gvar_ind][i]) ** 2)
                    lower_SD_alpha += sum((np.array(local_results[i][gvar_ind]) - np.array(local_vars_0[i][gvar_ind])) * (
                                hat_dual[gvar_ind][i][global_ind[i][gvar_ind]] - hat_dual_0[gvar_ind][i][global_ind[i][gvar_ind]]))

                    lower_MG_alpha += sum((np.array(local_results[i][gvar_ind]) - np.array(local_vars_0[i][gvar_ind])) ** 2)

                    upper_SD_beta += sum((global_vars["dual"][gvar_ind][i][global_ind[i][gvar_ind]] -
                                                     dual_0[gvar_ind][i][global_ind[i][gvar_ind]]) ** 2)
                    lower_SD_beta += sum((-global_vars["value"][gvar_ind][global_ind[i][gvar_ind]] + global_var_0[gvar_ind][
                        global_ind[i][gvar_ind]]) * (global_vars["dual"][gvar_ind][i][global_ind[i][gvar_ind]] -
                                                     dual_0[gvar_ind][i][global_ind[i][gvar_ind]]))
                    lower_MG_beta += sum((-global_vars["value"][gvar_ind][global_ind[i][gvar_ind]] + global_var_0[gvar_ind][
                        global_ind[i][gvar_ind]]) ** 2)

                #need abs?
                lower_SD_alpha = abs(lower_SD_alpha)
                lower_SD_beta = abs(lower_SD_beta)

                # compute spectral stepsizes
                hat_alpha_SD = upper_SD_alpha / lower_SD_alpha
                hat_alpha_MG = lower_SD_alpha / lower_MG_alpha
                if 2 * hat_alpha_MG > hat_alpha_SD:
                    hat_alpha = hat_alpha_MG
                else:
                    hat_alpha = hat_alpha_SD - hat_alpha_MG / 2

                hat_beta_SD = upper_SD_beta / lower_SD_beta
                hat_beta_MG = lower_SD_beta / lower_MG_beta
                if 2 * hat_beta_MG > hat_beta_SD:
                    hat_beta = hat_beta_MG
                else:
                    hat_beta = hat_beta_SD - hat_beta_MG / 2

                # estimate correlations
                alpha_corr = lower_SD_alpha / (math.sqrt(lower_MG_alpha) * math.sqrt(upper_SD_alpha))
                beta_corr = lower_SD_beta / (math.sqrt(lower_MG_beta) * math.sqrt(upper_SD_beta))

                wr += "\n  item " + str(gbv.item_list[i]) + str(upper_SD_alpha) + ", " + str(
                    lower_SD_alpha) + ", " + str(lower_MG_alpha) + ", " + str(upper_SD_beta) + ", " + str(
                    lower_SD_beta) + ", " + str(lower_MG_beta)

                # safeguarding adaptive penalty
                if alpha_corr > safety_guard_thres and beta_corr > safety_guard_thres:
                    hat_rho_i = math.sqrt(hat_alpha * hat_beta)
                elif alpha_corr > safety_guard_thres and beta_corr <= safety_guard_thres:
                    hat_rho_i = hat_alpha
                elif alpha_corr <= safety_guard_thres and beta_corr > safety_guard_thres:
                    hat_rho_i = hat_beta
                else:
                ...
                rho_sep[i] = max(min(hat_rho_i, (1 + conv_const / (iter_no ** 2)) * rho_sep[i]),
                                 rho_sep[i] / (1 + conv_const / (iter_no ** 2)))

            wr_s = open("rho_update", "a")
            wr_s.write(wr)
            wr_s.close()
        ...
        if iter_no % 1 == 0:
            wr = "\nIteration: " + str(iter_no) + "\nUpper bound : " + str(UB) + "\nLower bound : " + str(
                LB) + "\nRho max/min:" + str((max(rho_sep), min(rho_sep))) + "\nPrimal residual : " + str(
                np.round(primal_residual, 2)) + '\nDual residual : ' + str(
                np.round(dual_residual, 2)) + "\nIteration time : " + str(
    ...
    wr = "\nFinished !\n"
    if primal_residual <= primal_residual_tol and dual_residual <= dual_residual_tol:
        wr += "Break because of small residual!\n"
    wr += "\nUB: " + str(UB_ite) + "\nLB: " + str(LB_ite) + "\n"
    wr_s = open('results.txt', 'a')
    wr_s.write(wr)
    wr_s.close()
    ...
    #solver_start = time.time()
    #solver_obj = extensive_prob(solve_option=True, relax_option=False)
    # solver_obj = math.inf
