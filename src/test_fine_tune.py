...
def x_solve(dummy_item_index, rho, global_vars, quad_penalty=True, relax_option=True, penalty_mag=1e5, fixed_param=True, fixed_lag=6):
    '''
    ...
    prob.addConstrs((yUOi_p[j, t] - yUOi_m[j, t] == gp.quicksum(si[l + (t,)] for l in transit_list_i if l[0] == j) +
                     gp.quicksum(gbv.bom_dict[j][bk] * xCi[bk[0], j, t] for bk in gbv.bom_key[j] if bk[1] == item_ind and (bk[0], j) in prod_plant_i) + (zi_p[j, t] - zi_m[j, t]) +
                     gp.quicksum(gbv.alt_dict[jta] * rCi[jta] for jta in gbv.alt_list if
                                 (jta[0] == j) and (jta[1] == t) and (jta[2][0] == item_ind)) -
                     gp.quicksum(gbv.alt_dict[jta] * rCi[jta] for jta in gbv.alt_list if
                                 (jta[0] == j) and (jta[1] == t) and (jta[2][1] == item_ind))
                     for j in gbv.plant_list for t in gbv.period_list if gbv.bom_key.get(j, -1) != -1), name="output_item")
    prob.addConstrs((yUOi_p[j, t] - yUOi_m[j, t] == gp.quicksum(si[l + (t,)] for l in transit_list_i if l[0] == j)
                      + (
                                 zi_p[j, t] - zi_m[j, t]) +
                     gp.quicksum(gbv.alt_dict[jta] * rCi[jta] for jta in gbv.alt_list if
    ...
    prob.addConstrs((gp.quicksum(gbv.unit_cap[i_iter, j_iter, ct_iter] * xCi[i_iter, j_iter, t] for i_iter, j_iter, ct_iter in gbv.unit_cap.keys()
                                 if (j_iter == j) and (ct == ct_iter) and ((i_iter, j_iter) in prod_plant_i)) <= gbv.max_cap[ct, j][t]
                     for ct, j in gbv.max_cap.keys() for t in gbv.period_list if ((item_ind, j) in gbv.prod_key) and (t in gbv.max_cap[ct, j].keys())), name='capacity')
    prob.addConstrs((rCi[jta] <= vi[jta[0], jta[1] - 1] for jta in gbv.alt_list if jta[2][0] == item_ind), name='r_ub')
    prob.addConstrs(
        (rCi[jta] <= gbv.init_inv.get((jta[2][0], jta[0]), 0) for jta in gbv.alt_list if (jta[2][1] == item_ind) and (jta[1] == 1)),
        name='r_ub_rev_ini')
    ...
    obj_local_part -= gp.quicksum(
        global_vars["dual"][gvar_ind][dummy_item_index][xi_ind] * xCi[global_var_const["indices"][gvar_ind][xi_ind]] for xi_ind
        in global_ind_i[gvar_ind])

    gvar_ind = 1
    obj_local_part -= gp.quicksum(
        global_vars["dual"][gvar_ind][dummy_item_index][ri_ind] * rCi[global_var_const["indices"][gvar_ind][ri_ind]] for ri_ind
        in global_ind_i[gvar_ind])
    ...
    if quad_penalty:
        ...
        gvar_ind = 0
        obj_local_part += rho / 2 * gp.quicksum((global_vars["value"][gvar_ind][xi_ind] - xCi[global_var_const["indices"][gvar_ind][xi_ind]]) ** 2 \
    ...
    for xi_ind in global_ind_i[gvar_ind]:
        ...
        #wr = "Error: " + str(item_ind)
        #wr_s = open('error.txt', 'a')
        #wr_s.write(wr)
        #wr_s.close()
        #sys.exit(3)
...
def lb_add(global_vars):
    ...
    lb_add_mip.addConstrs((gp.quicksum(
        ...
        if (x_ind[glb_ind][1] == j) and (x_ind[glb_ind][2] == t)) <= gbv.max_cap[ct, j][t]
                           for ct, j in gbv.max_cap.keys() for t in gbv.period_list if t in gbv.max_cap[ct, j].keys()), name='capacity')
...
def z_solve_int_dp(local_results, rho, global_vars, var_threshold=1e-5, fixing_param=True, fixing_lag=6):
    '''
    ...
    local_dict["fixed"][gvar_ind] = np.minimum(
        local_dict["fixed"][gvar_ind] * x_bool_consistent + x_bool_consistent, np.ones(len(x_keys)) * global_counter[gvar_ind] * fixing_lag)

    return np.sqrt(dual_residual_sqr), primal_tolerance, local_dict, dp_time
    ...
    for gvar_ind in range(len(global_var_const["name"])):
        for i in range(I):
            if len(global_ind[i][gvar_ind]) > 0:
                ...
                residual_list[global_ind[i][gvar_ind]] = global_vars["value"][gvar_ind][global_ind[i][gvar_ind]] - local_array[global_ind[i][gvar_ind]]
...
def extensive_prob(solve_option=True, relax_option=False, solve_remaining_after_fixing=False, timelimit=30 * 60, global_vars=None,
                   fixing_lag=6, penalty_mag=1e5):
    # set up the extensive formulation
    global gbv, global_var_const, global_counter
    ...
    yUO_p = prob.addVars(gbv.item_list, gbv.plant_list, gbv.period_list, lb=0.0, name="yo_p")  # y^{O}_{ijt} for j,t
    yUO_m = prob.addVars(gbv.item_list, gbv.plant_list, gbv.period_list, lb=0.0, name="yo_m")
    xC = prob.addVars(gbv.prod_key, gbv.period_list, lb=0.0, name="x")  # x_{ijt} for i,j,t
    if relax_option:
        w = prob.addVars(gbv.prod_key, gbv.period_list, lb=0.0, name="w")  # w_{ijt} for i,j,t
    else:
        w = prob.addVars(gbv.prod_key, gbv.period_list, vtype=GRB.INTEGER, lb=0.0, name="w")  # w_{ijt} for i,j,t
    ...
    prob.addConstrs((yUO_p[i, j, t] - yUO_m[i, j, t] == gp.quicksum(
        s[l + (t,)] for l in gbv.transit_list if (l[0] == i) and (l[1] == j)) +
                     ...
                     for i in gbv.item_list for j in gbv.plant_list for t in gbv.period_list if gbv.bom_dict.get(j, -1) != -1),
                    name="output_item")
    prob.addConstrs((yUO_p[i, j, t] - yUO_m[i, j, t] == gp.quicksum(
        s[l + (t,)] for l in gbv.transit_list if (l[0] == i) and (l[1] == j)) + z_p[
                         i, j, t] - z_m[
                         i, j, t] +
                     gp.quicksum(gbv.alt_dict[jta] * rC[jta] for jta in gbv.alt_list if
                                 (jta[0] == j) and (jta[1] == t) and (jta[2][0] == i)) -
    ...
    prob.addConstrs((gp.quicksum(
        ...
        if (j_iter == j) and (ct == ct_iter)) <= gbv.max_cap[ct, j][t]
                     for ct, j in gbv.max_cap.keys() for t in gbv.period_list if t in gbv.max_cap[ct, j].keys()), name='capacity')
    prob.addConstrs(
        (rC[jta] <= v[jta[2][0], jta[0], jta[1] - 1] for jta in gbv.alt_list),
        name='r_ub')
    prob.addConstrs(
        ...
        name='yo_ub')
    prob.addConstrs(
        (xC[i, j, t] == w[i, j, t] * gbv.lot_size[i, j] for i, j in gbv.prod_key for t in gbv.period_list),
        name='batch')
    prob.addConstrs(
    ...
    if solve_remaining_after_fixing:
        ...
        for xi_ind in range(len(x_ind)):
            if global_vars["fixed"][gvar_ind][xi_ind] == global_counter[gvar_ind][xi_ind] * fixing_lag:
        ...
        for ri_ind in range(len(r_ind)):
            if global_vars["fixed"][gvar_ind][ri_ind] == global_counter[gvar_ind][ri_ind] * fixing_lag:
            ...
            for t in gbv.period_list:
                ...
                for i_iter, j_iter, ct_iter in gbv.unit_cap.keys():
                    if (j_iter == j) and (ct == ct_iter):
                        tem += gbv.unit_cap[i_iter, j_iter, ct_iter] * x_value[i_iter, j_iter, t]
                cap_used[ct, j, t] = tem
        '''
        return prob.objVal
    else:
...
# solve LP dual variables
def extensive_LP_prob(solve_option=True, penalty_mag=1e5):
    # set up the extensive formulation
    global gbv, global_var_const, sce_num

    prob = gp.Model("extensive_form_LP")
    prob.setParam("Threads", 1)
    ...
    if solve_option:
        ...
        global_var_value = [[x_val, r_val], prob.ObjVal]
...
def dual_LP_item(dummy_item_index, global_var_value, penalty_mag=1e5):
    ...
    # set the linear expression of the local part for the objective
    obj_local_part = gp.quicksum(gbv.holding_cost[item_ind] * vi[j, t] + penalty_mag * (zi_m[j, t] + yUOi_m[j, t]) \
                                 for j in gbv.plant_list for t in gbv.period_list) + gp.quicksum(gbv.penalty_cost[item_ind] * ui[t] for t in gbv.period_list) + \
                                gp.quicksum(gbv.transit_cost[(item_ind,) + l] * si[l + (t,)] for l in transit_list_i for t in gbv.period_list)
...
if __name__ == '__main__':
    ...
    '''
    extensive_LP_global_var_value = extensive_LP_prob()
    extensive_LP_obj = extensive_LP_global_var_value[1]
    local_LP_dual_vals = pool.map(partial(dual_LP_item, global_var_value=extensive_LP_global_var_value[0]), range(len(gbv.item_list)))

    #local_LP_obj = 0
    #for i in range(len(local_LP_dual_vals)):
    #  local_LP_obj += local_LP_dual_vals[i][2]
    ...
    # admm
    ini_start = time.time()
    local_results = pool.map(partial(x_solve, rho=rho, global_vars=global_vars, quad_penalty=False), range(len(gbv.item_list)))
    ...
    while primal_residual > 1e-3 or dual_residual > 1e-3:
        ...
        if (iter_no <= hparams.LP_warm_start_rounds) and (primal_residual > 10 * dual_residual): #and (primal_residual > primal_tol):
            #pass
            rho *= 2
        elif (iter_no <= hparams.LP_warm_start_rounds) and (dual_residual > 10 * primal_residual): #and (dual_residual > dual_tol):
            #pass
            rho /= 2
        ...
        total_var_size = 0
        for gvar_ind in range(len(global_var_const["name"])):
            fixed_var_num += np.count_nonzero(global_vars["fixed"][gvar_ind] == hparams.fixing_lag * global_counter[gvar_ind])
