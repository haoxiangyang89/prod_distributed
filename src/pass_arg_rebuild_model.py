    ...
    for i in gbv.item_list:
        ...
        for j in gbv.plant_list:
            if gbv.bom_key.get(j, -1) != -1:
                x_prod_list.extend([(it_id, j) for it_id in gbv.item_list if (it_id, i) in gbv.bom_key[j]])
...
def x_solve(dummy_item_index, rho, global_vars, quad_penalty=True, relax_option=True, penalty_mag=1e5, fixed_param=True, fixed_rounds=10):
    '''
    ...
    transit_time_dict = {transit_item: gbv.transit_time[item_ind, transit_item[0], transit_item[1]] for transit_item in transit_list_i}
    ...
    obj_local_part = gp.quicksum(gbv.holding_cost[item_ind] * vi[j, t] + penalty_mag * (zi_m[j, t] + yUOi_m[j, t]) \
                                 ...
                                 for t in gbv.period_list) + gp.quicksum(gbv.holding_cost[item_ind] * 1.05 * si[l + (t,)] * transit_time_dict[l] \
                                                                         for l in transit_list_i for t in gbv.period_list)

    gvar_ind = 0
    obj_local_part -= gp.quicksum(
        global_vars["dual"][gvar_ind][dummy_item_index][vi] * xCi[global_var_const["indices"][gvar_ind][vi]] for vi
        in global_ind_i[gvar_ind])

    gvar_ind = 1
    obj_local_part -= gp.quicksum(
        global_vars["dual"][gvar_ind][dummy_item_index][vi] * rCi[global_var_const["indices"][gvar_ind][vi]] for vi
        in global_ind_i[gvar_ind])
    ...
    try:
        ...
        print("Error: " + str(gbv.item_list[dummy_item_index]))
        sys.exit(3)
...
def x_solve_lb(dummy_item_index, global_vars, relax_option=True, penalty_mag=1e5):
    global gbv, global_ind, global_var_const
    item_ind = gbv.item_list[dummy_item_index]
    print(multiprocessing.current_process(), "\t(Lower bound)Setting up Model " + str(item_ind) + "!\n")
...
def extensive_prob(solve_option=True, relax_option=True, solve_remaining_after_fixing=False, global_vars=None, timelimit=1 * 60,
    ...
    prob.addConstrs((yUI[i, j, t] == gp.quicksum(
        s[l + (t - gbv.transit_time[l],)] for l in gbv.transit_list if (l[0] == i) and (l[2] == j)) +
                     xC[i, j, t - gbv.lead_time[i, j]] + gbv.external_purchase.get((i, j, t), 0) \
                     for i in gbv.item_list for j in gbv.plant_list for t in gbv.period_list if (i, j) in gbv.prod_key),
                    name='input_item')
    prob.addConstrs((yUI[i, j, t] == gp.quicksum(
        s[l + (t - gbv.transit_time[l],)] for l in gbv.transit_list if (l[0] == i) and (l[2] == j)) +
                    gbv.external_purchase.get((i, j, t), 0) \
                     for i in gbv.item_list for j in gbv.plant_list for t in gbv.period_list if (i, j) not in gbv.prod_key),
                    name='input_item')
    ...
    prob.addConstrs(
        ...
        name='r_ub')
    prob.addConstrs(
        (rC[jta] <= gbv.init_inv.get((jta[2][0], jta[0]), 0) for jta in gbv.alt_list if jta[1] == 1),
        name='r_ub_rev_ini')
    prob.addConstrs(
    ...
    if not (relax_option):
        ...
        if solve_remaining_after_fixing:
            ...
            for vi in range(len(x_ind)):
                if global_vars["fixed"][gvar_ind][vi] == fixing_rounds:
                    fixed_var = xC[global_var_const["indices"][gvar_ind][vi]]
                    fixed_var.lb = global_vars["value"][gvar_ind][vi]
                    fixed_var.ub = global_vars["value"][gvar_ind][vi]
    ...
    prob.setObjective(
        ...
        gp.quicksum(gbv.holding_cost[l[0]] * 1.05 * s[l + (t,)] * gbv.transit_time[l] for l in gbv.transit_list for t in gbv.period_list), GRB.MINIMIZE)
