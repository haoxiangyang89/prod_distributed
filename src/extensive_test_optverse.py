...
def extensive_prob(opt_data, solve_option = True, relax_option = False):
    ...
    ep.unmet_demand = op.Constraint(ep.I['i'], ep.T['t'], 
                        lhs = ep.u['i', 't'] - ep.u['i', ['t', - 1]] + op.sum_by(ep.z['i', 'j', 't'], by = [ep.J['j'],]),
                        sense = '=',
                        rhs = ep.real_demand['i', 't'], 
                        name='unmet_demand')
    ep.inventory = op.Constraint(ep.I['i'], ep.J['j'], ep.T['t'],
                        lhs = ep.v['i', 'j', 't'] - ep.v['i', 'j', ['t', - 1]] - ep.yUI['i', 'j', 't'] + ep.yUO['i', 'j', 't'],
                        sense = '=',
                        rhs = 0,
                        name='inventory')
    ep.input_item = op.Constraint(ep.I['i'], ep.J['j'], ep.T['t'],
                        lhs = ep.yUI['i', 'j', 't'],
                        sense = '=',
                        rhs = op.sum_by(e = ep.s['tr', ['t', - ep.transit_time['tr']]], cond = ep.transit_data['i','j','tr'], by = [ep.Tr["tr"],]) + \
                            ep.xC['i', 'j', ['t', - ep.lead_time['i', 'j']]] * ep.prod_key['i', 'j'] + ep.external_purchase['i', 'j', 't'],
                        name = 'input_item')
    ep.output_item = op.Constraint(ep.I['i'], ep.J['j'], ep.T['t'],
                        lhs = ep.yUO['i', 'j', 't'],
                        sense = '=',
                        rhs = op.sum_by(e = ep.s['tr','t'], cond = ep.r_transit_data['i', 'j', 'tr'], by = [ep.Tr["tr"],]) + \
                            op.sum_by(e = ep.xC['ii', 'j', 't'], cond = ep.bom_dict['ii', 'i', 'j'], by = [ep.I['ii'],]) + ep.z['i', 'j', 't'] + \
                            op.sum_by(e = ep.rC['i', 'ii', 'j', 't'], cond = ep.alt_dict['i', 'ii', 'j', 't'], by = [ep.I['ii'],]) - \
                            op.sum_by(e = ep.rC['ii', 'i', 'j', 't'], cond = ep.alt_dict['ii', 'i', 'j', 't'], by = [ep.I['ii'],]),
                        name = 'output_item')
    ep.capacity = op.Constraint(ep.J['j'], ep.Ct['ct'],
                        lhs = op.sum_by(e = ep.xC['i', 'j', 't'], cond = ep.unit_cap['i', 'j', 'ct'], by = [ep.I['i'], ]),
                        sense = '<=',
                        rhs = ep.max_cap['j', 'ct'],
                        name = 'capacity')
    ...
    ep.obj = op.Objective(expr = 
        op.sum_by(cond = ep.holding_cost['i'], e = ep.v['i', 'j', 't'], by = [ep.I['i'], ep.J['j'], ep.T['t']]) +
    ...
    if solve_option:
        ...
        instance.solve(solver=solver_optv, save_solution=True)
