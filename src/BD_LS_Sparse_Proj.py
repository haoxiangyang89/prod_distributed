...
if __name__ == "__main__":
    ...
    pp, pp_handle = master_prob(reg_method, level_obj_norm)
    ...
    while True:
        ...
        # obtaining the level and the new x
        rp = mp.relax()
        rp.setObjective(gp.quicksum((rp.getVarByName(f"x[{i},{j},{t}]") - best_incum[i, j, t]) ** 2 
        ...
        rp.addConstr(gp.quicksum(rp.getVarByName(f"theta[{i}]") for i in range(len(decomp_units))) <= level)
        
        rp.update()
        rp.optimize()
        x_vals_frac = {(i,j,t):rp.getVarByName(f"x[{i},{j},{t}]").X for i,j in prod_key for t in period_df.I}
        pp.setObjective(gp.quicksum((pp.getVarByName(f"x[{i},{j},{t}]") - x_vals_frac[i, j, t]) ** 2 
                                    for i, j in prod_key for t in period_df.I))
        pp.update()
        pp.optimize()
        x_vals_old = copy.deepcopy(x_vals)
        x_vals = {(i,j,t):pp.getVarByName(f"x[{i},{j},{t}]").X for i,j in prod_key for t in period_df.I}
        if x_vals_old == x_vals:
            level_fractile = level_fractile / 1.2
        else:
            level_fractile = 0.5
