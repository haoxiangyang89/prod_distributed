...
using JuMP, Gurobi, Random, Dates
...
# construct all subproblems
global subprob_list = Dict();
global prod_key_used_list = Dict();
for k in 1:length(ctx.decomp_units)
    subprob_list[k], prod_key_used_list[k] = _subproblem_common(ctx, k; sparse_cuts = sparse_cut_flag, with_capacity_neighbors = true, deviation_ub = :max_prod);
end
