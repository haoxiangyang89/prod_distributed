...
def x_solve(dummy_item_index, rho, global_vars, quad_penalty=True, relax_option=True, penalty_mag=1e5, fixed_param=True,
    ...
    relate_item_i = []
    for e in prod_plant_i:
        if (e[0] != item_ind) and (e[0] not in relate_item_i):
            relate_item_i.append(e[0])
    ...
    for i in gbv.item_list:
        for j in gbv.plant_list:
            if (i, j) not in gbv.prod_key:
                for t in gbv.period_list:
                    xCi[i, j, t] = 0.0
            else:
                if(i, j) not in prod_plant_i:
                    for t in gbv.period_list:
                        xCi[i, j, t] = gbv.max_prod.get((i, j), math.inf)
    ...
    prob.addConstrs((yUIi[j, t] == gp.quicksum(
        si[l + (t - gbv.transit_time[(item_ind,) + l],)] for l in transit_list_i if l[1] == j) +
                     ...
                     for j in gbv.plant_list for t in gbv.period_list if (item_ind, j) in prod_plant_i),
    ...
    prob.addConstrs((yUOi_p[j, t] - yUOi_m[j, t] == gp.quicksum(si[l + (t,)] for l in transit_list_i if l[0] == j) +
                     gp.quicksum(
                         gbv.bom_dict[j][bk] * xCi[bk[0], j, t] for bk in gbv.bom_key[j] if (bk[1] == item_ind) and ((bk[0], j) in prod_plant_i)) + (
                             zi_p[j, t] - zi_m[j, t]) +
    ...
    prob.addConstrs(
        ...
        name='w_ub')
    prob.addConstrs((zi_p[j, t] - zi_m[j, t] <= vi[j, t - 1] for j in gbv.plant_list for t in gbv.period_list),
                    name='z_ub')
    prob.addConstrs((si[l + (t,)] <= vi[l[0], t - 1] for l in transit_list_i for t in gbv.period_list), name='l_ub')


    # logical constraints
    for j in gbv.plant_list:
        if gbv.bom_key.get(j, -1) != -1:
            prob.addConstrs((gp.quicksum(xCi[bk[0], j, t] * gbv.bom_dict[j][bk] for bk in gbv.bom_key[j] if
                                         (bk[1] == item_ind) and ((bk[0], j) in gbv.prod_key)) <= vi[j, t - 1] for t in
                             gbv.period_list), name='x_ub')

    prob.addConstrs((vi[j, t] <= gbv.X_outside_ub_plant[item_ind, j][t] + gp.quicksum(
        xCi[item_ind, j, tp] for tp in gbv.period_list if tp <= t - gbv.lead_time[item_ind, j])
                     + gp.quicksum(
        xCi[item_ind, j_prime, tp] for j_prime in gbv.plant_list for tp in gbv.period_list if
        (item_ind, j_prime) in gbv.prod_key
        and ((item_ind, j_prime, j) in gbv.transit_list) and (
                tp <= t - gbv.lead_time[item_ind, j_prime] - gbv.transit_time[item_ind, j_prime, j])) for j in
                     gbv.plant_list for t in gbv.period_list if (item_ind, j) in gbv.prod_key), name='v_ub')
    prob.addConstrs((vi[j, t] <= gbv.X_outside_ub_plant[item_ind, j][t]
                     + gp.quicksum(
        xCi[item_ind, j_prime, tp] for j_prime in gbv.plant_list for tp in gbv.period_list if
        (item_ind, j_prime) in gbv.prod_key
        and ((item_ind, j_prime, j) in gbv.transit_list) and (
                tp <= t - gbv.lead_time[item_ind, j_prime] - gbv.transit_time[item_ind, j_prime, j])) for j in
                     gbv.plant_list for t in gbv.period_list if (item_ind, j) not in gbv.prod_key), name='v_ub')

    prob.addConstrs(
        (gbv.composition_list[i, ip] * gp.quicksum(
            xCi[i, j, tp] for tp in gbv.period_list if tp <= t) <=
         gbv.X_outside_ub_plant[ip, j][t - 1] + gp.quicksum(
            xCi[ip, j, tp] for tp in gbv.period_list if tp <= t - 1 - gbv.lead_time[ip, j])
         + gp.quicksum(
            xCi[item[0], item[1], tp] for item in gbv.prod_key for tp in gbv.period_list if
            (item[0] == ip) and ((ip, item[1], j) in gbv.transit_list) and (
                    tp <= t - 1 - gbv.lead_time[item] - gbv.transit_time[ip, item[1], j]))
         for i, j in prod_plant_i for t in gbv.period_list for ip in gbv.subsidiary_list[i]), name='plant_ub'
    )


    prob.addConstrs(gp.quicksum(
        gbv.composition_list[i_prime, item_ind] * xCi[i_prime, j_prime, t] for
        i_prime, j_prime in prod_plant_i \
        if i_prime in gbv.parent_list[item_ind]) <= gbv.X_outside_ub[item_ind][t - 1] + \
                          gp.quicksum(xCi[item_ind, j_prime, t_prime] for j_prime in gbv.plant_list for t_prime in gbv.period_list if
                                      (t_prime <= t - 1 - gbv.lead_time[
                                          item_ind, j_prime]) and (item_ind, j_prime) in gbv.prod_key)
                          for t in gbv.period_list
                          )
    prob.addConstrs(gp.quicksum(
        gbv.composition_list[ip, item_ind] * xCi[ip, j, t] for
        ip in gbv.item_list if ((ip, j) in gbv.prod_key) \
        and ((ip, item_ind) in gbv.bom_key[j])) <=
                          gbv.X_outside_ub_plant[item_ind, j][t - 1] +
                          gp.quicksum(xCi[item_ind, j, tp] for tp in gbv.period_list if
                                      tp <= t - 1 - gbv.lead_time[item_ind, j])
                          + gp.quicksum(xCi[item_ind, jp, tp] for jp in gbv.plant_list for tp in gbv.period_list if
                                        ((item_ind, jp) in gbv.prod_key) and ((item_ind, jp, j) in gbv.transit_list) and (
                                                tp <= t - 1 - gbv.lead_time[item_ind, jp] - gbv.transit_time[item_ind, jp, j]))
                          for j in gbv.bom_key.keys() for t in gbv.period_list
                          )
    ...
    obj_local_part = gp.quicksum(gbv.holding_cost[item_ind] * vi[j, t] + gbv.penalty_cost[item_ind] * (zi_m[j, t] + yUOi_m[j, t]) \
...
def x_solve_dual(dummy_item_index, dual_vars, rho, primal_vars, quad_penalty=True, relax_option=True, penalty_mag=1e5,
    ...
    x_prob.addConstrs(
        (wi[i, j, t] <= gbv.max_prod[i, j] / gbv.lot_size[i, j] for i, j in prod_plant_i for t in gbv.period_list),
        name='w_ub')
    x_prob.addConstrs((zi_p[j, t] - zi_m[j, t] <= vi[j, t - 1] for j in gbv.plant_list for t in gbv.period_list),
                    name='z_ub')
    x_prob.addConstrs((si[l + (t,)] <= vi[l[0], t - 1] for l in transit_list_i for t in gbv.period_list), name='l_ub')
...
def x_solve_ub(dummy_item_index, global_vars, relax_option=True, penalty_mag=1e5):
    ...
    prob.addConstrs((yUOi_p[j, t] - yUOi_m[j, t] == gp.quicksum(si[l + (t,)] for l in transit_list_i if l[0] == j) +
                     gp.quicksum(
                         gbv.bom_dict[j][bk] * xCi[bk[0], j, t] for bk in gbv.bom_key[j] if
                         (bk[1] == item_ind) and ((bk[0], j) in prod_plant_i)) + (
                             zi_p[j, t] - zi_m[j, t]) +
                     gp.quicksum(gbv.alt_dict[jta] * rCi[jta] for jta in gbv.alt_list if
                                 (jta[0] == j) and (jta[1] == t) and (jta[2][0] == item_ind)) -
                     gp.quicksum(gbv.alt_dict[jta] * rCi[jta] for jta in gbv.alt_list if
    ...
    prob.addConstrs(gp.quicksum(
        ...
        if i_prime in gbv.parent_list[item_ind]) <= gbv.X_outside_ub[item_ind][t - 1] + \
                    gp.quicksum(
                        xCi[item_ind, j_prime, t_prime] for j_prime in gbv.plant_list for t_prime in gbv.period_list if
                        (t_prime <= t - 1 - gbv.lead_time[
                            item_ind, j_prime]) and (item_ind, j_prime) in gbv.prod_key)
                    for t in gbv.period_list
                    )
    prob.addConstrs(gp.quicksum(
        ...
        and ((ip, item_ind) in gbv.bom_key[j])) <=
                    ...
                    + gp.quicksum(xCi[item_ind, jp, tp] for jp in gbv.plant_list for tp in gbv.period_list if
                                  ((item_ind, jp) in gbv.prod_key) and ((item_ind, jp, j) in gbv.transit_list) and (
                                          tp <= t - 1 - gbv.lead_time[item_ind, jp] - gbv.transit_time[
                                      item_ind, jp, j]))
                    for j in gbv.bom_key.keys() for t in gbv.period_list
                    )
...
def extensive_prob(solve_option=True, relax_option=False, solve_remaining_after_fixing=False, timelimit=30 * 60, global_vars=None,
                   penalty_mag=1e5, fixing_lag=6, fixing_round=10):
    ...
    ext_prob.addConstrs(
        (z_p[i, j, t] - z_m[i, j, t] <= v[i, j, t - 1] for i in gbv.item_list for j in gbv.plant_list for t in
         gbv.period_list),
        name='z_ub')
    ext_prob.addConstrs((s[l + (t,)] <= v[l[0], l[1], t - 1] for l in gbv.transit_list for t in gbv.period_list),
                    name='l_ub')
    ...
    if solve_remaining_after_fixing:
        ...
        for xi_ind in range(len(x_ind)):
            if global_vars["fixed"][gvar_ind][xi_ind] == global_counter[gvar_ind][xi_ind] * fixing_lag:   #global_counter[gvar_ind][xi_ind] * fixing_lag
                fixed_var = xC[global_var_const["indices"][gvar_ind][xi_ind]]
                fixed_var.lb = global_vars["value"][gvar_ind][xi_ind]
                fixed_var.ub = global_vars["value"][gvar_ind][xi_ind]

        gvar_ind = 1
        r_ind = gbv.alt_list
        for ri_ind in range(len(r_ind)):
            if global_vars["fixed"][gvar_ind][ri_ind] == global_counter[gvar_ind][ri_ind] * fixing_lag:    #global_counter[gvar_ind][ri_ind] * fixing_lag:
                fixed_var = rC[global_var_const["indices"][gvar_ind][ri_ind]]
                fixed_var.lb = global_vars["value"][gvar_ind][ri_ind]
                fixed_var.ub = global_vars["value"][gvar_ind][ri_ind]
    ...
    ext_prob.setObjective(
        gp.quicksum(
            gbv.holding_cost[i] * v[i, j, t] + gbv.penalty_cost[i] * (yUO_m[i, j, t] + z_m[i, j, t]) for i in
            gbv.item_list for j in gbv.plant_list for t in gbv.period_list) +
...
def dual_LP_item(dummy_item_index, global_var_value, penalty_mag=1e5):
    ...
    xCi = dual_prob.addVars(gbv.prod_key, gbv.period_list, lb=0.0, name="x")  # x_{ijt} for i,j,t (x copy)
    wi = dual_prob.addVars(gbv.prod_key, gbv.period_list, lb=0.0, name="w")  # w_{ijt} for i,j,t
    ...
    dual_prob.addConstrs((yUOi_p[j, t] - yUOi_m[j, t] <= vi[j, t - 1] for j in gbv.plant_list for t in gbv.period_list),
                    name='yo_ub')
    dual_prob.addConstrs(
        (xCi[i, j, t] == wi[i, j, t] * gbv.lot_size[i, j] for i, j in gbv.prod_key for t in gbv.period_list),
        name='batch')
    dual_prob.addConstrs(
        (wi[i, j, t] <= gbv.max_prod[i, j] / gbv.lot_size[i, j] for i, j in gbv.prod_key for t in gbv.period_list),
        name='w_ub')
    dual_prob.addConstrs((zi_p[j, t] - zi_m[j, t] <= vi[j, t - 1] for j in gbv.plant_list for t in gbv.period_list),
                    name='z_ub')
    dual_prob.addConstrs((si[l + (t,)] <= vi[l[0], t - 1] for l in transit_list_i for t in gbv.period_list), name='l_ub')
    ...
    for t in gbv.period_list:
        for j in gbv.plant_list:
            obj += gbv.holding_cost[item_ind] * vi[j, t].X + gbv.penalty_cost[item_ind] * (zi_m[j, t].X + yUOi_m[j, t].X)
...
def z_solve_int_solver_bound(local_results, rho, global_vars, var_threshold=1e-5, fixing_param=True, fixing_lag=6, fixing_round=10):
    '''
    solve z problem with integer variables: solve for the global variables
    local_results: the output from the x problem
    rho: second order penalty coefficient
    ...
    # upper node is restricted by lower node a path in bom tree
    global_mip.addConstrs(
        (gbv.composition_list[i, ip] * gbv.lot_size[i, j] * gp.quicksum(
            w_vars[glb_ind] for glb_ind in range(len(x_keys)) if
            (x_ind[glb_ind][0] == i) and ((x_ind[glb_ind][1] == j)) and (x_ind[glb_ind][2] <= t)) <=
         gbv.X_outside_ub_plant[ip, j][t - 1] + gp.quicksum(
            x_vars[glb_ind] for glb_ind in range(len(x_keys)) if
            (x_ind[glb_ind][0] == ip) and (x_ind[glb_ind][1] == j) and (
                        x_ind[glb_ind][2] <= t - 1 - gbv.lead_time[ip, j]))
         + gp.quicksum(
            x_vars[glb_ind] for glb_ind in range(len(x_keys)) if (x_ind[glb_ind][0] == ip)
            and ((ip, x_ind[glb_ind][1], j) in gbv.transit_list) and (
                    x_ind[glb_ind][2] <= t - 1 - gbv.lead_time[ip, x_ind[glb_ind][1]] - gbv.transit_time[
                ip, x_ind[glb_ind][1], j]))
         for i, j in gbv.prod_key for t in gbv.period_list for ip in gbv.subsidiary_list[i]), name='plant_ub'
    )

    global_mip.addConstrs(gp.quicksum(
        gbv.composition_list[x_ind[glb_ind][0], i] * gbv.lot_size[x_ind[glb_ind][0], x_ind[glb_ind][1]] * w_vars[glb_ind] for
        glb_ind in
        range(len(x_keys)) \
        if (x_ind[glb_ind][0] in gbv.parent_list[i]) and (x_ind[glb_ind][2] == t)) <= gbv.X_outside_ub[i][t - 1] + \
                          gp.quicksum(x_vars[glb_ind] for glb_ind in range(len(x_keys)) if
                                      (x_ind[glb_ind][0] == i) and (x_ind[glb_ind][2] <= t - 1 - gbv.lead_time[
                                          x_ind[glb_ind][0], x_ind[glb_ind][1]]))
                          for i in gbv.item_list for t in gbv.period_list
                          )
    global_mip.addConstrs(gp.quicksum(
        gbv.composition_list[x_ind[glb_ind][0], i] * gbv.lot_size[x_ind[glb_ind][0], j] * w_vars[glb_ind] for
        glb_ind in
        range(len(x_keys)) \
        if (x_ind[glb_ind][1] == j) and ((x_ind[glb_ind][0], i) in gbv.bom_key[j]) and (x_ind[glb_ind][2] == t)) <=
                          gbv.X_outside_ub_plant[i, j][t - 1] +
                          gp.quicksum(x_vars[glb_ind] for glb_ind in range(len(x_keys)) if
                                      (x_ind[glb_ind][0] == i) and (x_ind[glb_ind][1] == j) and (
                                                  x_ind[glb_ind][2] <= t - 1 - gbv.lead_time[i, j]))
                          + gp.quicksum(x_vars[glb_ind] for glb_ind in range(len(x_keys)) if
                                        (x_ind[glb_ind][0] == i) and (
                                                    (i, x_ind[glb_ind][1], j) in gbv.transit_list) and (
                                                    x_ind[glb_ind][2] <= t - 1 - gbv.lead_time[
                                                x_ind[glb_ind][0], x_ind[glb_ind][1]] - gbv.transit_time[
                                                        i, x_ind[glb_ind][1], j]))
                          for i in gbv.item_list for j in gbv.bom_key.keys() for t in gbv.period_list
                          )
    ...
    x_bool_consistent = (x_value_consistent <= var_threshold)
    local_dict["fixed"][gvar_ind] = np.minimum(local_dict["fixed"][gvar_ind] * x_bool_consistent + x_bool_consistent,
                                               np.ones(len(x_keys)) * global_counter[gvar_ind] * fixing_lag)   # * global_counter[gvar_ind] * fixing_lag
    ...
    r_bool_consistent = (r_value_consistent <= var_threshold)
    local_dict["fixed"][gvar_ind] = np.minimum(local_dict["fixed"][gvar_ind] * r_bool_consistent + r_bool_consistent,
                                               np.ones(len(r_keys)) * global_counter[gvar_ind] * fixing_lag)    #global_counter[gvar_ind] * fixing_lag
...
if __name__ == '__main__':
    ...
    z_solve = eval("z_solve_int_solver_bound")
    ...
    ini_start = time.time()
    local_results = pool.map(partial(x_solve, rho=rho, global_vars=global_vars, quad_penalty=True),
    ...
    while primal_residual > 1e-3 or dual_residual > 1e-3:
        ...
        for gvar_ind in range(len(global_var_const["name"])):
            fixed_var_num += np.count_nonzero(
                global_vars["fixed"][gvar_ind] == hparams.fixing_lag * global_counter[gvar_ind])   #hparams.fixing_lag * global_counter[gvar_ind]
        ...
        if iter_no % 5 == 0:
            ...
            wr_s = open('logic_only_sub.txt', 'a')
            wr_s.write(wr)
            wr_s.close()
    ...
    solver_start = time.time()
    solver_obj = extensive_prob(solve_option=True, relax_option=False, solve_remaining_after_fixing=False, timelimit=admm_time)
    # solver_obj = math.inf
