...
def global_const_init(gbv):
    ...
    for i in gbv.item_list:
        ...
        x_name = ["x[{},{},{}]".format(x_prod[0], x_prod[1], t) for x_prod in gbv.prod_key for t in gbv.period_list]
...
def x_solve(dummy_item_index, rho, global_vars, quad_penalty=True, relax_option=True, penalty_mag=1e5, fixed_param=True,
    ...
    if relax_option:
        wi = prob.addVars(gbv.prod_key, gbv.period_list, lb=0.0, name="w")  # w_{ijt} for i,j,t
    else:
    ...
    prob.addConstrs((vi[j, t] <= gbv.X_outside_ub_plant[item_ind, j][t] + gp.quicksum(
        ...
        ((item_ind, j_prime, j) in gbv.transit_list) and (
                tp <= t - gbv.lead_time[item_ind, j_prime] - gbv.transit_time[item_ind, j_prime, j])) for j in
                     gbv.plant_list for t in gbv.period_list), name='v_ub')
    ...
    prob.addConstrs(gp.quicksum(
        gbv.composition_list[i_prime, i] * xCi[i_prime, j_prime, t] for
        i_prime, j_prime in gbv.prod_key \
        if i_prime in gbv.parent_list[i]) <= gbv.X_outside_ub[i][t - 1] + \
                          gp.quicksum(xCi[i, j_prime, t_prime] for j_prime in gbv.plant_list for t_prime in gbv.period_list if
                                      (t_prime <= t - 1 - gbv.lead_time[
                                          i, j_prime]))
                          for i in gbv.item_list for t in gbv.period_list
                          )
    prob.addConstrs(gp.quicksum(
        gbv.composition_list[ip, i] * xCi[ip, j, t] for
        ip in gbv.item_list if ((ip, j) in gbv.prod_key) \
        and ((ip, i) in gbv.bom_key[j])) <=
                          gbv.X_outside_ub_plant[i, j][t - 1] +
                          gp.quicksum(xCi[i, j, tp] for tp in gbv.period_list if
                                      tp <= t - 1 - gbv.lead_time[i, j])
                          + gp.quicksum(xCi[i, jp, tp] for jp in gbv.plant_list for tp in gbv.period_list if
                                        ((i, jp) in gbv.prod_key) and ((i, jp, j) in gbv.transit_list) and (
                                                tp <= t - 1 - gbv.lead_time[i, jp] - gbv.transit_time[i, jp, j]))
    ...
    if fixed_param:
        ...
        gvar_ind = 1
        for ri_ind in global_ind_i[gvar_ind]:
            if global_vars["fixed"][gvar_ind][ri_ind] == fixing_round:       #global_counter[gvar_ind][ri_ind] * fixed_lag:
...
def extensive_prob(solve_option=True, relax_option=False, solve_remaining_after_fixing=False, timelimit=30 * 60, global_vars=None,
    ...
    if solve_remaining_after_fixing:
        ...
        for xi_ind in range(len(x_ind)):
            if global_vars["fixed"][gvar_ind][xi_ind] == fixing_round:   #global_counter[gvar_ind][xi_ind] * fixing_lag
        ...
        for ri_ind in range(len(r_ind)):
            if global_vars["fixed"][gvar_ind][ri_ind] == fixing_round:    #global_counter[gvar_ind][ri_ind] * fixing_lag:
...
def z_solve_int_solver_bound(local_results, rho, global_vars, var_threshold=1e-5, fixing_param=True, fixing_lag=6, fixing_round=10):
    ...
    x_bool_consistent = (x_value_consistent <= var_threshold)
    local_dict["fixed"][gvar_ind] = np.minimum(local_dict["fixed"][gvar_ind] * x_bool_consistent + x_bool_consistent,
                                               np.ones(len(x_keys)) * fixing_round)   # * global_counter[gvar_ind] * fixing_lag
    ...
    r_bool_consistent = (r_value_consistent <= var_threshold)
    local_dict["fixed"][gvar_ind] = np.minimum(local_dict["fixed"][gvar_ind] * r_bool_consistent + r_bool_consistent,
                                               np.ones(len(r_keys)) * fixing_round)    #global_counter[gvar_ind] * fixing_lag
    ...
    while primal_residual > 1e-3 or dual_residual > 1e-3:
        ...
        for gvar_ind in range(len(global_var_const["name"])):
            fixed_var_num += np.count_nonzero(
                global_vars["fixed"][gvar_ind] == hparams.fixing_rounds)   #hparams.fixing_lag * global_counter[gvar_ind]
