...
def master_prob(reg_method, level_obj_norm):
    ...
    decomp_dict = {i:[i] for i in item_df.I if (len(df_alt[df_alt.I == i]) > 0) or (len(df_alt[df_alt.II == i]) > 0) or \
                                                (len(df_prod[df_prod.I == i]) > 0) or (len(df_bom[df_bom.I == i]) > 0) or (len(df_bom[df_bom.II == i]) > 0)}
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for item in sub_opt_results:
            ...
            if feas_gen:
                if np.sum(np.abs(np.floor(list(feas_dual.values())))) > 0:
                    mp.addConstr(gp.quicksum(np.floor(feas_dual[i, j, t]) * mp.getVarByName(f"x[{i},{j},{t}]") for i, j, t in feas_dual.keys()) <=
                        np.floor(sum(feas_dual[i, j, t] *  x_vals[i, j, t] for i, j, t in feas_dual.keys()) - feas_value))
                else:
