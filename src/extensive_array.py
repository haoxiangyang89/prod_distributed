...
def extensive():
    ...
    mp.addConstrs((yUIi[i, j, t] == gp.quicksum(
        si[df_transit.Tr[l], t - int(transit_time[l])] for l in df_transit.index[(df_transit.I == i) & (df_transit.JJ == j)]) +
                   (x[i, j, t - int(lead_time[i, j])] * lot_size[i, j] if (i, j) in prod_key else 0.0) + external_purchase[i, j, t] +
