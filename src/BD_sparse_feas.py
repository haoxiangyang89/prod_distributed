...
def master_prob(sol_method, cut_opt, reg_method, level_obj_norm):
    ...
    mp.addConstrs((gp.quicksum(
        unit_cap[i_iter, j_iter, ct_iter] * x[i_iter, j_iter, t] * lot_size[i_iter, j_iter] for i_iter, j_iter, ct_iter
        in unit_cap.keys() if (j_iter == j) and (ct == ct_iter) and (i_iter, j_iter) in prod_key) <= max_cap[ct, j, t]
...
def sub_primal_prob(unit_ind_list, x_vals, cut_type):
    ...
    zi = sp.addVars(unit_ind_list, plant_df.I, period_df.I, lb=0.0, name="z")  # z^+_{ijt} for j,t
    ...
    yUOi = sp.addVars(unit_ind_list, plant_df.I, period_df.I, lb=0.0, name="yo")  # y^{O,+}_{ijt} for j,t
    ...
    sp.addConstrs((ui[i, t] - ui[i, t - 1] + gp.quicksum(zi[i, j, t] for j in plant_df.I) \
    ...
    sp.addConstrs((yUIi[i, j, t] == gp.quicksum(
        si[transit_list_i.Tr[l], t - transit_time[l]] for l in
    ...
    sp.addConstrs((yUOi[i, j, t] == gp.quicksum(
        si[transit_list_i.Tr[l], t] for l in transit_list_i.index[(transit_list_i.I == i) & (transit_list_i.J == j)]) +
                   gp.quicksum(df_bom.V[bk] * xCi[df_bom.I[bk], j, t] for bk in
                               df_bom.index[(df_bom.J == j) & (df_bom.II == i)]) + zi[i, j, t] +
                   gp.quicksum(alt_i.V[jta] * rCi[jta] for jta in
    ...
    sp.addConstrs((gp.quicksum(
        ...
        (j_iter == j) and (ct == ct_iter) and (i_iter, j_iter) in prod_key) <= max_cap[ct, j, t]
                   for ct, j, t in max_cap.keys()), name='capacity')
    ...
    for i, j in prod_key:
        for t in period_df.I:
            if cut_type == "feas_cut":
                x_dev_p = sp.addVars(prod_key, period_df.I, lb=0.0, name="x_dev_p")
                x_dev_m = sp.addVars(prod_key, period_df.I, lb=0.0, name="x_dev_m")

                sub_fix_x[i, j, t] = sp.addConstr(xCi[i, j, t] + x_dev_p[i, j, t] - x_dev_m[i, j, t] == x_vals[i, j, t] * lot_size[i, j])
            elif cut_type == "opt_cut":
                sub_fix_x[i, j, t] = sp.addConstr(xCi[i, j, t] == x_vals[i, j, t] * lot_size[i, j])
    # set up the subproblem specific objective
    if cut_type == "feas_cut":
        obj = gp.quicksum(x_dev_p[i, j, t] + x_dev_m[i, j, t] for i, j in prod_key for t in period_df.I)
    elif cut_type == "opt_cut":
        obj = gp.quicksum(
...
def sub_dual_prob(unit_ind_list, x_vals, cut_type):
    global df_alt, df_transit, df_bom, item_df, plant_df, period_df
    global prod_key, lot_size, max_prod, unit_cap, max_cap, lead_time, real_demand, external_purchase, \
        init_inv, unit_cap, max_cap, holding_cost, penalty_cost, transit_cost, transit_time
    ...
    if cut_type == "feas_cut":
        sd.addConstrs(-dual_v[i, j, t] + dual_v[i, j, t + 1] + dual_OUB[i, j, t + 1] +
                      gp.quicksum(
                          dual_rUB[jta] for jta in
                          alt_i.index[(alt_i.J == j) & (alt_i.Ti == t + 1) & (alt_i.I == i)]) >= 0
                      for i in unit_ind_list for j in plant_df.I for t in
                      range(min(period_df.I), max(period_df.I)))
        sd.addConstrs(
            -dual_v[i, j, max(period_df.I)] >= 0 for i in unit_ind_list for j in plant_df.I)
    elif cut_type == "opt_cut":
        sd.addConstrs(holding_cost[i] - dual_v[i, j, t] + dual_v[i, j, t + 1] + dual_OUB[i, j, t + 1] +
                      gp.quicksum(
                          dual_rUB[jta] for jta in alt_i.index[(alt_i.J == j) & (alt_i.Ti == t + 1) & (alt_i.I == i)]) >= 0
                      for i in unit_ind_list for j in plant_df.I for t in
                      range(min(period_df.I), max(period_df.I)))
        sd.addConstrs(
    ...
    # coeff_u >= 0
    if cut_type == "feas_cut":
        sd.addConstrs(-dual_u[i, t] + dual_u[i, t + 1] >= 0 for i in unit_ind_list for t in
                      range(min(period_df.I), max(period_df.I)))
        sd.addConstrs(-dual_u[i, max(period_df.I)] >= 0 for i in unit_ind_list)
    elif cut_type == "opt_cut":
    ...
    if cut_type == "feas_cut":
        sd.addConstrs(
            dual_I[transit_list_i.I[l], transit_list_i.JJ[l], t + transit_time[l]] + \
        ...
        sd.addConstrs(dual_O[transit_list_i.I[l], transit_list_i.J[l], t] >= 0
                      for l in transit_list_i.index for t in
                      range(max(period_df.I) - transit_time[l] + 1, max(period_df.I) + 1))
    elif cut_type == "opt_cut":
        sd.addConstrs(
            transit_cost[transit_list_i.Tr[l]] + dual_I[transit_list_i.I[l], transit_list_i.JJ[l], t + transit_time[l]] + \
            dual_O[transit_list_i.I[l], transit_list_i.J[l], t] >= 0
            for l in transit_list_i.index for t in
            range(min(period_df.I), max(period_df.I) - transit_time[l] + 1)
            if transit_time[l] <= max(period_df.I) - min(period_df.I))
    ...
    sd.addConstrs(-dual_rUB[jta] + alt_i.V[jta] * (
            dual_I[alt_i.II[jta], alt_i.J[jta], alt_i.Ti[jta]] + dual_O[alt_i.I[jta], alt_i.J[jta], alt_i.Ti[jta]]) >= 0
                  for jta in alt_i.index)
    ...
    sd.addConstrs(-dual_xUB[i, j, t] - pi[i, j, t] + dual_I[i, j, t + lead_time[i, j]]
                  ...
                  + gp.quicksum(df_bom.V[bk] * dual_O[df_bom.II[bk], j, t] for bk in
                                df_bom.index[(df_bom.J == j) & (df_bom.I == i) & (df_bom.II.isin(unit_ind_list))])
                  >= 0 for i, j in prod_key for t in period_df.I)

    # coeff_x_dev >= 0
    if cut_type == "feas_cut":
        sd.addConstrs(1 - pi[i, j, t] >= 0 for i, j in prod_key for t in period_df.I)
        sd.addConstrs(1 + pi[i, j, t] >= 0 for i, j in prod_key for t in period_df.I)

    obj = gp.quicksum(real_demand[i, t] * dual_u[i, t] for i in unit_ind_list for t in period_df.I) \
          + gp.quicksum(
        init_inv[i, j] * (dual_v[i, j, min(period_df.I)] + dual_OUB[i, j, min(period_df.I)]) for i in unit_ind_list for
        j in
        plant_df.I) \
...
def master_sub_loop():
    ...
    global cut_opt, dual_opt, reg_method
    global level_fractile, level_obj_norm
    global best_incum
    global period_df, prod_key

    pool = Pool(min(len(decomp_units), 30))  # 30
    ...
    while True:
        ...
        ############################## feasibility cut ##############################
        if dual_opt == "gurobi_dual":
            sub_feas_results = pool.map(partial(sub_primal_prob, x_vals=x_vals, cut_type="feas_cut"), decomp_units)

            dual_sub_feas_results = pool.map(partial(sub_dual_prob, x_vals=x_vals, cut_type="feas_cut"), decomp_units)
            dual_right = True
            wr = "\nDual right? "
            for k in range(len(decomp_units)):
                primal_obj = sub_feas_results[k][0]
                dual_obj = dual_sub_feas_results[k][0]
                if abs(primal_obj - dual_obj) > 1e-4:
                    dual_right = False
                    wr += str(dual_right) + " ----- Sub " + str(decomp_units[k]) + ": primal " + str(primal_obj) + ", dual " + str(dual_obj)
                    break
            if dual_right:
                wr += str(dual_right)
            wr_s = open("benders.txt", "a")
            ...
            sub_feas_results = pool.map(partial(sub_dual_prob, x_vals=x_vals, cut_type="feas_cut"), decomp_units)
        ...
        for k in range(len(decomp_units)):
            ...
            if sub_obj > 1e-4:
                ...
                mp.addConstr(0 >= sub_obj + gp.quicksum(
        ...
        if x_feas_flag:
            if dual_opt == "gurobi_dual":
                sub_opt_results = pool.map(partial(sub_primal_prob, x_vals=x_vals, cut_type="opt_cut"), decomp_units)
                '''
                dual_sub_opt_results = pool.map(partial(sub_dual_prob, x_vals=x_vals), decomp_units)
                dual_right = True
                wr = "\nDual right? "
                for k in range(len(decomp_units)):
                    primal_obj = sub_opt_results[k][0]
                    dual_obj = dual_sub_opt_results[k][0]
                    if abs(primal_obj - dual_obj) > 1e-4:
                        dual_right = False
                        wr += str(dual_right) + " ----- Sub " + str(decomp_units[k]) + ": primal " + str(primal_obj) + ", dual " + str(dual_obj)
                        break
                ...
                sub_opt_results = pool.map(partial(sub_dual_prob, x_vals=x_vals, cut_type="opt_cut"), decomp_units)
            ...
            # wr = "\nPrimal objective: "
            # for k in range(len(decomp_units)):
            #    wr += "\n\tSubPrimalprob " + str(decomp_units[k]) + " : Objective " + str(sub_opt_results[k][0])
            # wr_s = open("benders.txt", "a")
            # wr_s.write(wr)
            # wr_s.close()

            ub_cand = 0
            if cut_opt == "single_cut":
                cut_x_coeff = {(i, j, t): 0 for i, j in prod_key for t in period_df.I}
            for k in range(len(decomp_units)):
                sub_result = sub_opt_results[k]
                ...
                if cut_opt == "multi_cut":
                    ...
                    for i, j in prod_key:
                        for t in period_df.I:
                            cut_x_coeff[i, j, t] += sub_dual[i, j, t]

            if cut_opt == "single_cut":
                if theta_vals < ub_cand:
                    mp.addConstr(mp.getVarByName(f"theta") >= ub_cand + gp.quicksum(
                        cut_x_coeff[i, j, t] * (mp.getVarByName(f"x[{i},{j},{t}]") - x_vals[i, j, t])
                        for i, j in prod_key for t in period_df.I))
                    wr = "\nOptimality cut:" + "\n\ttheta" + " >= " + str(ub_cand)
                    for i, j in prod_key:
                        for t in period_df.I:
                            wr += " + (" + str(cut_x_coeff[i, j, t]) + ")*" + "(x" + str([i, j, t]) + "-" + str(
                                x_vals[i, j, t]) + ")"
                    wr_s = open("../benders.txt", "a")
                    wr_s.write(wr)
                    wr_s.close()
...
if __name__ == "__main__":
    ...
    data_folder = "../data/pilot_test"  # pilot_test/fine_tune
    # df_cost, df_alt, df_iniInv, df_external, df_unitCap, df_prod, df_transit, df_rtransit, \
