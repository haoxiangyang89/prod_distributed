...
def global_const_init(gbv):
    ...
    for i in gbv.item_list:
        ...
        for it,j in gbv.prod_key:
            if (it in gbv.precursor_list[i]) or (it in gbv.subsidiary_list[i]):
                x_prod_list.append((it,j))
...
def x_solve(dummy_item_index, rho, global_vars, logic_cons=False, quad_penalty=True, relax_option=True, penalty_mag=1e5, fixed_param=True, fixed_lag=6):
    '''
    ...
    for it,j in gbv.prod_key:
        if (it in gbv.precursor_list[item_ind]) or (it in gbv.subsidiary_list[item_ind]):
            prod_plant_i.append((it,j))
    ...
    if logic_cons:
        # logical constraints
        # for item_ind, its precursor summed up should not exceed its inventory level
        prob.addConstrs((gp.quicksum(gbv.composition_list[prod_plant_i[pp_ind][0],item_ind] * gbv.lot_size[prod_plant_i[pp_ind][0],prod_plant_i[pp_ind][1]] * \
                                    wi[prod_plant_i[pp_ind][0],prod_plant_i[pp_ind][1],t] for pp_ind in range(len(prod_plant_i)) \
                                    if (prod_plant_i[pp_ind][0] in gbv.precursor_list[item_ind])) <= \
                        gbv.X_outside_ub[item_ind][gbv.period_list.index(t)] + \
                        gp.quicksum(xCi[item_ind, j, tp] for i,j in gbv.prod_key for tp in gbv.period_list if (i == item_ind) and (tp <= t - gbv.lead_time[i, j])) \
            for t in gbv.period_list), name = 'overall_ub'
        )

        for i,j in gbv.prod_key:
            if i == item_ind:
                sub_zero_bound = np.minimum.reduce([np.floor(gbv.X_outside_ub_plant[ip,j]/(gbv.composition_list[i,ip] * gbv.lot_size[i,j])) \
                                                    for ip in gbv.subsidiary_list[i] if gbv.degree_list[gbv.item_list.index(ip)] == 0])
                
                # obtain the degree 0 bounds for each higher level item
                prob.addConstrs(gp.quicksum(wi[i,j,tp] for tp in gbv.period_list if tp <= t) <= \
                                    sub_zero_bound[gbv.period_list.index(t)] for t in gbv.period_list)
                # obtain the subsidiary bound for each higher level item
                prob.addConstrs(gp.quicksum(wi[i,j,tp] for tp in gbv.period_list if tp <= t) <= \
                                    (gbv.X_outside_ub_plant[ip,j][gbv.period_list.index(t)] + \
                                    gp.quicksum(xCi[ip,ji,tp] for ji in gbv.item_plant[ip] for tp in gbv.period_list \
                                                    if tp <= t - gbv.lead_time[ip,ji]))/(gbv.composition_list[i,ip] * gbv.lot_size[i,j]) \
                                    for ip in gbv.subsidiary_list[i] for t in gbv.period_list \
                                        if gbv.degree_list[gbv.item_list.index(ip)] > 0)
    ...
    obj_local_part = gp.quicksum(gbv.holding_cost[item_ind] * vi[j, t] + penalty_mag * (zi_m[j, t] + yUOi_m[j, t]) \
                     ...
                     gp.quicksum(gbv.transit_cost[item_ind, l[0], l[1]] * si[l + (t,)] for l in transit_list_i for t in gbv.period_list)
...
def x_solve_dual(dummy_item_index, dual_vars, rho, primal_vars, logic_cons = False, quad_penalty=True, relax_option=True, penalty_mag=1e5, fixed_param=True, fixed_lag=6):
    '''
    A reformulation of the sub problem to guarantee feasibility for the sub problems
    item_ind: the index of sub-problem
    return: the subproblem built with Gurobi
    ...
    x_prob.addConstrs((gp.quicksum(
        ...
        if (j_iter == j) and (ct == ct_iter) and ((i_iter, j_iter) in prod_plant_i)) <= gbv.max_cap[ct, j][t]
                     for ct, j in gbv.max_cap.keys() for t in gbv.period_list if
                     ((item_ind, j) in gbv.prod_key) and (t in gbv.max_cap[ct, j].keys())), name='capacity')
    x_prob.addConstrs((rCi[jta] <= vi[jta[0], jta[1] - 1] for jta in gbv.alt_list if jta[2][0] == item_ind), name='r_ub')
    x_prob.addConstrs(
        (rCi[jta] <= gbv.init_inv.get((jta[2][0], jta[0]), 0) for jta in gbv.alt_list if
         (jta[2][1] == item_ind) and (jta[1] == 1)),
        name='r_ub_rev_ini')
    ...
    if logic_cons:
        # logical constraints
        # for item_ind, its precursor summed up should not exceed its inventory level
        x_prob.addConstrs((gp.quicksum(gbv.composition_list[prod_plant_i[pp_ind][0],item_ind] * gbv.lot_size[prod_plant_i[pp_ind][0],prod_plant_i[pp_ind][1]] * \
                                    wi[prod_plant_i[pp_ind][0],prod_plant_i[pp_ind][1],t] for pp_ind in range(len(prod_plant_i)) \
                                    if (prod_plant_i[pp_ind][0] in gbv.precursor_list[item_ind])) <= \
                        gbv.X_outside_ub[item_ind][gbv.period_list.index(t)] + \
                        gp.quicksum(xCi[item_ind, j, tp] for i,j in gbv.prod_key for tp in gbv.period_list if (i == item_ind) and (tp <= t - gbv.lead_time[i, j])) \
        ...
        for i,j in gbv.prod_key:
            if i == item_ind:
                ...
                # obtain the degree 0 bounds for each higher level item
                x_prob.addConstrs(gp.quicksum(wi[i,j,tp] for tp in gbv.period_list if tp <= t) <= \
                                    sub_zero_bound[gbv.period_list.index(t)] for t in gbv.period_list)
                # obtain the subsidiary bound for each higher level item
                x_prob.addConstrs(gp.quicksum(wi[i,j,tp] for tp in gbv.period_list if tp <= t) <= \
                                    (gbv.X_outside_ub_plant[ip,j][gbv.period_list.index(t)] + \
    ...
    gvar_ind = 0
    obj_local_part -= gp.quicksum(
        dual_vars[gvar_ind][xi_ind] * xCi[global_var_const["indices"][gvar_ind][global_ind_i[gvar_ind][xi_ind]]] for xi_ind
        in range(len(global_ind_i[gvar_ind])))

    gvar_ind = 1
    obj_local_part -= gp.quicksum(
        dual_vars[gvar_ind][xi_ind] * rCi[global_var_const["indices"][gvar_ind][global_ind_i[gvar_ind][ri_ind]]] for ri_ind
        in range(len(global_ind_i[gvar_ind])))
    ...
    if quad_penalty:
        # if the objective contains the quadratic penalty term (bundle method)
        gvar_ind = 0
        obj_local_part += rho / 2 * gp.quicksum((primal_vars[gvar_ind][xi_ind] - xCi[global_var_const["indices"][gvar_ind][xi_ind]]) ** 2 \
                                                for xi_ind in global_ind_i[gvar_ind])
...
def x_solve_ub(dummy_item_index, global_vars, relax_option = True, penalty_mag = 1e5):
    ...
    # set up the subproblem specific objective
    penalty_mag = gbv.penalty_cost[item_ind]
    obj_local_part = gp.quicksum(gbv.holding_cost[item_ind] * vi[j, t] + penalty_mag * (zi_m[j, t] + yUOi_m[j, t]) \
                    for j in gbv.plant_list for t in gbv.period_list) + gp.quicksum(gbv.penalty_cost[item_ind] * ui[t] \
                    for t in gbv.period_list) + gp.quicksum(gbv.transit_cost[item_ind, l[0], l[1]] * si[l + (t,)] for l in transit_list_i for t in gbv.period_list)
...
def extensive_prob(solve_option = True, relax_option = False, time_limit = 6000):
    # set up the extensive formulation
    # set up the extensive formulation
    global gbv
    ext_prob = gp.Model("extensive_form")
    ext_prob.setParam("Threads", 1)
    ext_prob.setParam("TimeLimit", time_limit)
...
def z_solve_int_solver_bound(local_results, rho, global_vars, var_threshold=1e-5, fixing_param=True, fixing_lag=6):
    ...
    '''
    global_mip.addConstrs(
        (gbv.composition_list[x_ind[glb_ind][0],ip] * gbv.lot_size[x_ind[glb_ind][0], x_ind[glb_ind][1]] * gp.quicksum(w_vars[x_ind.index((x_ind[glb_ind][0],x_ind[glb_ind][1],tau))] for tau in gbv.period_list if tau <= x_ind[glb_ind][2]) <= \
        gbv.X_outside_ub_plant[ip, x_ind[glb_ind][1]][gbv.period_list.index(x_ind[glb_ind][2])] + gp.quicksum(x_vars[x_ind.index((item[0],item[1],tp))] for item in gbv.prod_key for tp in gbv.period_list if (item[0] == ip) and (tp <= x_ind[glb_ind][2] - gbv.lead_time[item]))\
        for glb_ind in range(len(x_keys)) for ip in gbv.subsidiary_list[x_ind[glb_ind][0]]), name = 'plant_ub'
    )
    '''
    
    # for each item i, its precursor summed up should not exceed its inventory level
    global_mip.addConstrs((gp.quicksum(gbv.composition_list[x_ind[glb_ind][0],i] * gbv.lot_size[x_ind[glb_ind][0],x_ind[glb_ind][1]] * w_vars[glb_ind] for glb_ind in range(len(x_keys)) \
        if (x_ind[glb_ind][0] in gbv.precursor_list[i]) and (x_ind[glb_ind][2] == t)) <= gbv.X_outside_ub[i][gbv.period_list.index(t)] + \
            gp.quicksum(x_vars[glb_ind] for glb_ind in range(len(x_keys)) if (x_ind[glb_ind][0] == i) and (x_ind[glb_ind][2] <= t - gbv.lead_time[i, x_ind[glb_ind][1]]))
        for i in gbv.item_list for t in gbv.period_list), name = 'overall_ub'
    )
    ...
    for i,j in gbv.prod_key:
        ...
        # obtain the degree 0 bounds for each higher level item
        global_mip.addConstrs(gp.quicksum(w_vars[x_ind.index((i,j,tp))] for tp in gbv.period_list if tp <= t) <= \
                              sub_zero_bound[gbv.period_list.index(t)] for t in gbv.period_list)
        # obtain the subsidiary bound for each higher level item
        global_mip.addConstrs(gp.quicksum(w_vars[x_ind.index((i,j,tp))] for tp in gbv.period_list if tp <= t) <= \
                              (gbv.X_outside_ub_plant[ip,j][gbv.period_list.index(t)] + \
                              gp.quicksum(x_vars[x_ind.index((ip,ji,tp))] \
                                          for ji in gbv.item_plant[ip] for tp in gbv.period_list \
                                            if tp <= t - gbv.lead_time[ip,ji]))/(gbv.composition_list[i,ip] * gbv.lot_size[i,j]) \
                            for ip in gbv.subsidiary_list[i] for t in gbv.period_list \
                                if gbv.degree_list[gbv.item_list.index(ip)] > 0)
...
if __name__ == '__main__':
    ...
    ext_prob, ext_prob_handle = extensive_prob(False, False, 100)
    ext_prob.optimize()
    ...
    #ini_start = time.time()
    #local_results = pool.map(partial(x_solve, rho=rho, global_vars=global_vars, quad_penalty=True), range(len(gbv.item_list)))
    ...
    while primal_residual > 1e-3 or dual_residual > 1e-3:
        ...
        if iter_no > hparams.LP_warm_start_rounds:
            ...
            if not (np.inf in ub_results):
                if sum(ub_results) < UB:
                    current_best_sol = global_vars["value"]
