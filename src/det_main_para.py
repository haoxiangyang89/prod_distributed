...
def z_solve_int(local_results, rho, var_threshold = 1e-6):
    '''
    solve z problem: solve for the global variables
    local_results: the output from the x problem
    rho: second order penalty coefficient
    ...
    global_mip.addConstrs((gp.quicksum(gbv.unit_cap[x_ind[glb_ind][0],x_ind[glb_ind][1]] * x_vars[glb_ind] for glb_ind in range(len(x_keys)) \
