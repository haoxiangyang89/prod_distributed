...
def pi_iter(unit_ind_list, x_vals, sub_obj, penalty_mag=1e5):
    # Attempt to find the flat constraints with better numerical properties
    global period_df
    global prod_key

    smcp = gp.Model()
    thetav = smcp.addVar(ub=10*sub_obj, name="thetav")
    pi = smcp.addVars(prod_key, period_df.I, lb=-float('inf'), name="beta")

    smcp.setObjective(thetav, GRB.MAXIMIZE)
...
def worker_init(data):
    ...
    # item clustering such that items with substitutution relationship are in the same cluster
    global alt_item_pair_list, alt_items_list, decomp_units
    alt_item_pair_list = list(zip(df_alt.I, df_alt.II))
    alt_item_pair_list = [tuple(sorted(tuple_)) for tuple_ in alt_item_pair_list]
    alt_item_pair_list = list(dict.fromkeys(alt_item_pair_list))
...
def master_sub_loop(data):
    global mp, mp_handle
    global max_iter, iter_num, best_lb, best_ub
    global cut_opt, dual_opt, reg_method, smc_switch
    global level_fractile, level_obj_norm
    global best_incum
    period_df = data["period_df"]
    df_alt = data["df_alt"]
    ...
    pool = Pool(6, initializer=worker_init, initargs=(data,))   # 30
    ...
    while True:
        ...
        # optimality cut       
        if dual_opt == "gurobi_dual":
            sub_opt_results = pool.map(partial(sub_primal_prob, x_vals=x_vals), decomp_units)
            '''
            ...
            sub_opt_results = pool.map(partial(sub_dual_prob, x_vals=x_vals), decomp_units)
        ...
        if cut_opt == "single_cut":
            cut_x_coeff = np.zeros((I_norm, J_norm, T_norm))
        for k in range(len(decomp_units)):
            sub_result = sub_opt_results[k]
            ...
            if cut_opt == "multi_cut":
                if theta_vals[k] < sub_obj:
                    ...
                    pi_opt = pi_iter(decomp_units[k], x_vals, sub_obj)
                    vLD, x_diff = sub_primal_Lag(decomp_units[k], x_vals, pi_opt)
