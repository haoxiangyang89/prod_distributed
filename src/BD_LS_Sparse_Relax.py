...
if __name__ == "__main__":
    # read in data
    data_folder = "../data/medium_test_fine_tune"  # pilot_test/fine_tune
    ...
    try:
        df_cost.PC = item_data.penalty_cost
    except:
        pass
    gbv = {}
    gbv["df_cost"] = df_cost
    ...
    # add the necessary variables for norm-1 regularization
    x_abs = mp.addVars(prod_key, period_df.I, lb = 0.0, name = "x_abs")
    abs_cons_p = mp.addConstrs(x_abs[i,j,t] - mp.getVarByName(f"x[{i},{j},{t}]") >= 0.0 for i,j in prod_key for t in period_df.I)
    abs_cons_m = mp.addConstrs(x_abs[i,j,t] + mp.getVarByName(f"x[{i},{j},{t}]") >= 0.0 for i,j in prod_key for t in period_df.I)
    mp_level_con = mp.addConstr(gp.quicksum(mp.getVarByName(f"theta[{i}]") for i in range(len(decomp_units))) <= 1e20)
    ...
    while True:
        ...
        sub_opt_results = pool.map(partial(pi_iter_sparse, x_vals=x_vals, theta_vals=theta_vals, lag_cuts=False, feas_cuts=feas_bool), range(len(decomp_units)))
        ub_cand = 0
        ...
        mp.setObjective(gp.quicksum( mp_handle[1][thetaKey] for thetaKey in mp_handle[1].keys()), GRB.MINIMIZE)
        mp.Params.TimeLimit = 10 * 60
        mp_level_con.rhs = best_ub
        mp.update()
        mp.optimize()
        ...
        # obtaining the level and the new x using 1-norm regularization
        # mp.setObjective(gp.quicksum((mp.getVarByName(f"x[{i},{j},{t}]") - best_incum[i, j, t]) ** 2 
        #                             for i, j in prod_key for t in period_df.I))
        for i,j in prod_key:
            for t in period_df.I:
                abs_cons_p[i,j,t].rhs = -best_incum[i, j, t]
                abs_cons_m[i,j,t].rhs = best_incum[i, j, t]
        mp.setObjective(gp.quicksum(x_abs[i,j,t] for i, j in prod_key for t in period_df.I), GRB.MINIMIZE)

        # level set initialization
        if best_ub < math.inf and best_lb > -math.inf:
            level = best_lb + level_fractile * (best_ub - best_lb)
        mp.Params.TimeLimit = 1 * 60
        mp_level_con.rhs = level
        mp.update()
        mp.optimize()
        
        x_vals = {(i,j,t):mp.getVarByName(f"x[{i},{j},{t}]").X for i,j in prod_key for t in period_df.I}
