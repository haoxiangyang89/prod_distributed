def solve_global():
    ...
    gbv.U = np.sum(gbv.rho * gbv.UC - gbv.Lam, axis=1) / (gbv.rho * gbv.K)
    gbv.X = np.sum(gbv.rho * gbv.XC - gbv.Mu, axis=2) / (gbv.rho * gbv.K)
    gbv.R = np.sum(gbv.rho * gbv.RC - gbv.Ksi, axis=2) / (gbv.rho * gbv.K)

    # Update dual variables
    for i in range(gbv.K):
        for ii in range(gbv.K):
            for t in range(gbv.N):
                gbv.Lam[i][ii][t] += gbv.rho * (gbv.U[i][t] - gbv.UC[i][ii][t])
    for i in range(gbv.K):
        for ii in range(gbv.K):
