...
def solver():
    ...
    prob.addConstrs((yUO[i][j][t] >= 0 for i in range(data["K"]) for j in range(data["M"]) for t in range(data["N"])),
                    name='1k1')
    prob.addConstrs(
        (data["v0"][i][j] - yUO[i][j][0] >= 0 for i in range(data["K"]) for j in range(data["M"])),
        name='3k2')
    prob.addConstrs(
        (v[i][j][t - 1] - yUO[i][j][t] >= 0 for i in range(data["K"]) for j in range(data["M"]) for t in range(1, data["N"])),
        name='1k2')
...
def model_var_setup(i):
    ...
    #                name='3k1')
    prob.addConstrs(
        (data["v0"][i][j] - yUOi[j, 0] >= 0 for j in range(data["M"])),
        name='3k2')
    prob.addConstrs(
        (vi[j, t - 1] - yUOi[j, t] >= 0 for j in range(data["M"]) for t in range(1, data["N"])),
        name='3k3')
...
def model_item_sub(i):
    #global item_model, item_var
    #data, var, i = a[0], a[1], a[2]
    ...
    result.extend([time_x, time_x_model_var_setup, time_x_set_obj, time_x_opt, start, end_item])

    return result

def model_item_global():  # solve 2nd block of ADMM, which is to update the global variables
    K, M, N = var["X"].shape
...
def comp_obj():  # Compute the objective corresponding to a solution
    VV = np.sum(var["V"], axis=2)
    ob = np.sum(np.multiply(data["P"], var["U"])) + np.sum(np.multiply(data["H"], VV))

    return ob

def go():  # solve items' problems in parallel
    ...
    #items = [(var, i) for i in list(range(data['K']))]
    # items = []
    # for i in list(range(data['K'])):
    #    a = [data, var, i]
    #    items.append(a)
    para_start = time.time()
    # results = pool.starmap_async(model_item_sub, items)
    # results = pool.starmap(model_item_sub, items)
    results = pool.map(model_item_sub, list(range(data['K'])))
    # results = pool.imap_unordered(model_item_sub, items)
    para_end = time.time()
    '''
    results = []
    time_x = []
    ...
    for i in range(data['K']):
        U_new[i] = results[i][0]  # result[0]
        S_new[i] = results[i][1]  # result[1]
        Z_new[i] = results[i][2]  # result[2]
        V_new[i] = results[i][3]  # result[3]
        YUI_new[i] = results[i][4]  # result[4]
        YUO_new[i] = results[i][5]  # result[5]
        XC_new[:, i, :, :] = results[i][6]  # result[6]
        if len(data["out_place"][i]) > 0:
            RC1_new[data["out_place"][i]] = results[i][7]  # result[7]
            if len(data["in_place"][i]) > 0:
                RC2_new[data["in_place"][i]] = results[i][8]  # result[8]
        elif len(data["out_place"][i]) == 0:
            if len(data["in_place"][i]) > 0:
                RC2_new[data["in_place"][i]] = results[i][7]  # result[7]

        time_x.append(results[i][-6])  # result[-7]
        time_x_model_var_setup.append(results[i][-5])  # result[-6]
        time_x_set_obj.append(results[i][-4])  # result[-5]
        time_x_opt.append(results[i][-3])  # result[-4]
        time_x_start.append(results[i][-2])  # result[-3]
        time_x_end.append(results[i][-1])  # result[-2]

    parallel_time = para_end - para_start

    # return time_x, sum(time_x)
    return [[U_new, S_new, Z_new, V_new, YUI_new, YUO_new, XC_new, RC1_new, RC2_new],
            [time_x, time_x_model_var_setup, time_x_set_obj, time_x_opt, parallel_time, time_x_start, time_x_end,
             para_start, para_end]]
...
def ADMM(solver_ob, solver_sol):
    ...
    while pr > p_tol or dr > d_tol:  # 当primal/dual residual < tolerance时终止循环
        ...
        if pr > 5 * dr and pr > p_tol:
        #if pr > 5 * dr:
            var["rho"] *= 2
        #elif dr > 5 * pr:
        elif dr > 5 * pr and dr > d_tol:
            var["rho"] /= 2
        ...
        if var["ite"] % 10 == 0:
            ...
            wr = "Iteration : " + str(var["ite"]) + '\nRho : ' + str(var["rho"]) + "\nPrimal residual : " + str(
                ...
                parallel_time) + "\n x_sub_start(earliest,latest): " + str(min(time_x_start)) + '\t' + str(
                max(time_x_start)) + "\n x_sub_end(earliest,latest): " + str(min(time_x_end)) + '\t' + str(
                max(time_x_end)) + "\n parallel start : " + str(para_start) + "\n parallel end : " + str(para_end) + "\n"
...
def init_child(share_memory):
    global var        #use global context (for each process) to pass arr to do_stuff
    var = share_memory
...
if __name__ == '__main__':
    ...
    solver_ob, solver_sol = solver()
    end_solver = time.time()

    # ADMM
    pool = Pool(processes=min(mp.cpu_count(), data["K"]), initializer=init_child, initargs=(var,))
    start_ADMM = time.time()
    admm_ob = ADMM(solver_ob, solver_sol)
    end_ADMM = time.time()
